    ...
    async with async_playwright() as p:
            ...
            if await skill_input.is_visible(timeout=3000):
                ...
                for skill in skills:
                    ...
                    add_skill_btn = page.locator(
                        'button[type="submit"], button:has-text("Add Skill")'
                    ).first
        ...
        if await location.is_visible(timeout=3000):
            if not await location.input_value():
                ...
                await asyncio.sleep(1)
        role = page.locator(
            'input[placeholder*="role" i], input[placeholder*="Product Manager" i]'
        ).first
        ...
        save_prefs = page.locator(
            'button:has-text("Save preferences"), button[data-onboarding-next]'
        ).first
        ...
        if await goals.is_visible(timeout=5000):
            await goals.fill(
                "Looking for senior engineering roles in fast-growing tech companies where I can lead technical initiatives"
            )
