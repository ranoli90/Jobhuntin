...
async def complete_full_onboarding():
    async with async_playwright() as p:
        ...
        if 'preferences' in page_text.lower() or 'location' in page_text.lower() or 'role' in page_text.lower():
            print("  ✓ Confirmed on Preferences step")
            
            # Fill Location
            location_input = page.locator('input[placeholder*="location" i], input[placeholder*="Remote" i], input[role="combobox"]').first
        ...
        if 'resume' in page_text.lower() or 'upload' in page_text.lower() or 'linkedin' in page_text.lower():
            ...
            # Fill LinkedIn URL
            linkedin_input = page.locator('input[type="url"], input[placeholder*="linkedin" i]').first
            ...
            for i, inp in enumerate(all_inputs[:10]):
                try:
                    if await inp.is_visible(timeout=1000):
                        ...
                        print(f"    Input {i}: type='{input_type}', name='{name}', placeholder='{placeholder[:40]}'")
                except:
                    pass
                ...
                try:
                    if await inp.is_visible(timeout=2000):
                        ...
                        if 'skill' in placeholder.lower() or 'skill' in name.lower():
                            skills_input = inp
                            print(f"  Found skills input by placeholder/name: '{placeholder}'")
                            break
                except:
                    continue
            ...
            if not skills_input:
                try:
                    ...
                    if await add_skill_elem.is_visible(timeout=2000):
                        ...
                        parent = add_skill_elem.locator('xpath=ancestor::*[contains(@class, "skill") or contains(@class, "add")]')
                ...
                for inp in all_inputs:
                    try:
                        if await inp.is_visible(timeout=1000):
                            input_type = await inp.get_attribute('type') or ''
                            placeholder = await inp.get_attribute('placeholder') or ''
                            if input_type in ['text', ''] and 'email' not in placeholder.lower() and 'phone' not in placeholder.lower() and 'location' not in placeholder.lower() and 'name' not in placeholder.lower():
                                skills_input = inp
