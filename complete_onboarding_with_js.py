...
async def complete_with_js():
    async with async_playwright() as p:
        ...
        if console_warnings:
            print(f"  ⚠ Found {len(console_warnings)} console warnings (showing first 5):")
        ...
        if current_step == 1:
            ...
            if 'welcome' in page_text.lower() or 'start setup' in page_text.lower():
                await click_button_robust('button:has-text("Start setup")', 'Start setup')
        ...
        if current_step == 2 or current_step == 1:
            ...
            if 'preferences' in page_text.lower() or 'location' in page_text.lower():
                ...
                await click_button_robust('button:has-text("Save preferences")', 'Save preferences')
                await asyncio.sleep(8)
                current_step = await get_step_number()
        ...
        if current_step == 3 or current_step == 2:
            ...
            if 'resume' in page_text.lower():
                ...
                await click_button_robust('button:has-text("Skip for now")', 'Skip for now')
                await asyncio.sleep(8)
                current_step = await get_step_number()
        ...
        if current_step == 4 or 'skill' in page_text.lower():
            ...
            if 'python' not in page_text.lower() and 'javascript' not in page_text.lower():
                # Click "Add your first skill" button
                add_btn_clicked = await click_button_robust('button:has-text("Add your first skill"), button:has-text("Add a missing skill")', 'Add skill')
                if add_btn_clicked:
            ...
            save_btn = page.locator('button:has-text("Save & Continue"), button:has-text("Continue")').first
                ...
                for btn in all_buttons:
                    try:
                        if await btn.is_visible(timeout=1000):
                            ...
                            if ('save' in text.lower() or 'continue' in text.lower()) and 'skip' not in text.lower() and 'back' not in text.lower():
                                disabled = await btn.get_attribute('disabled')
                                if not disabled:
            ...
            if save_clicked:
                ...
                if new_step and new_step > 4:
                    ...
                    skip_clicked = await click_button_robust('button:has-text("Skip for now")', 'Skip for now')
                    if skip_clicked:
                        await asyncio.sleep(10)
                        await page.wait_for_load_state('networkidle')
                        new_step = await get_step_number()
                        ...
                        if new_step and new_step > 4:
                            print(f"  ✓ Skills step skipped, advanced to step {new_step}")
                        else:
        ...
        if current_step == 6 or 'work style' in page_text.lower():
            ...
            await asyncio.sleep(2)
            save_clicked = await click_button_robust('button:has-text("Save Work Style"), button:has-text("Save work style"), button:has-text("Continue")', 'Save Work Style')
            if save_clicked:
                await asyncio.sleep(10)
                await page.wait_for_load_state('networkidle')
                new_step = await get_step_number()
        ...
        if current_step == 7 or 'career' in page_text.lower() or 'goal' in page_text.lower():
            ...
            if not textarea_filled:
                ...
                if await contenteditable.is_visible(timeout=5000):
                    await contenteditable.fill("Looking for senior engineering roles in fast-growing tech companies where I can lead technical initiatives and mentor junior developers")
                    print("  ✓ Filled contenteditable")
                    textarea_filled = True
            
            # Click Continue
            await asyncio.sleep(2)
            continue_clicked = await click_button_robust('button:has-text("Continue"), button:has-text("Next")', 'Continue')
            if continue_clicked:
                await asyncio.sleep(10)
                await page.wait_for_load_state('networkidle')
                new_step = await get_step_number()
        ...
        if current_step == 8 or 'ready' in page_text.lower() or 'complete' in page_text.lower() or 'finish' in page_text.lower():
            print("  ✓ On Ready step")
