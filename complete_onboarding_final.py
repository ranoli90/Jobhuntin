    ...
    async with async_playwright() as p:
        ...
        if current_step == 2:
            try:
                ...
                save_btn = None
                for selector in ['button:has-text("Save preferences")', 'button:has-text("Continue")', 'button[data-onboarding-next]']:
                    try:
        ...
        if current_step == 4:
            ...
            try:
                ...
                inputs = await page.locator('input[type="text"], input[placeholder*="skill" i], input[placeholder*="name" i]').all()
                if len(inputs) > 0:
            ...
            if save_btn and await save_btn.is_visible(timeout=5000):
                ...
                if new_step and new_step > 4:
                    ...
                    print(f"  ⚠ Step didn't advance (still {new_step}), waiting longer...")
                    await asyncio.sleep(5)
                    new_step = await get_step_number()
        ...
        if 'contact' in page_text.lower() or 'confirm' in page_text.lower() or 'first name' in page_text.lower():
            print("  ✓ Step 5 is ConfirmContact")
            
            # Check if fields are already filled
            first_name = page.locator('input[name*="first"], input[placeholder*="first" i]').first
            last_name = page.locator('input[name*="last"], input[placeholder*="last" i]').first
