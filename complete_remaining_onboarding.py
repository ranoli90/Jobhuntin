    ...
    async with async_playwright() as p:
        ...
        if "skill" in page_text.lower() and (
            ...
            skills_present = (
                ...
                or "python" in page_text.lower()
                or "react" in page_text.lower()
            )
                ...
                for skill in skills:
                    try:
                        ...
                        for inp in all_inputs:
                            try:
                                if await inp.is_visible(timeout=2000):
                                    ...
                                    if input_type in ["text", ""] and (
                                        "skill" in placeholder.lower()
                                        or "react" in placeholder.lower()
                                        or "python" in placeholder.lower()
                                        or not placeholder
                                    ):
                                        skills_input = inp
                                        break
                        ...
                        if skills_input:
                            ...
                            print(
                                f"  ⚠ Could not find input for {skill}, may already be added"
                            )
            ...
            if not continue_clicked:
                ...
                print(
                    "  Trying to click any available button (not Back/Restart/Add)..."
                )
        ...
        if "work style" in page_text.lower() or "workstyle" in page_text.lower():
            ...
            print(
                f"  Answered {selected_count} questions, filled {filled} text fields, checked {checked} checkboxes"
            )
        ...
        if "/app/dashboard" in final_url:
            ...
            try:
                jobs_link = page.locator(
                    'a:has-text("Jobs"), button:has-text("Browse Jobs")'
                ).first
