...
async def comprehensive_e2e_test():
    async with async_playwright() as p:
        ...
        await page.goto('http://localhost:5173/login', wait_until='networkidle', timeout=60000)
        ...
        try:
            ...
            for btn in all_buttons:
                if await btn.is_visible(timeout=1000):
                    ...
                    if 'Continue' in text and 'Google' not in text and not is_disabled and aria_disabled != 'true':
                        submit_btn = btn
                        break
        except:
            pass
        ...
        if submit_btn:
            ...
            try:
                submit_btn = page.locator('button:has-text("Continue"):not(:has-text("Google"))').first
                    ...
                    with open('/tmp/backend.log', 'r') as f:
                        ...
                        for line in reversed(lines[-200:]):
                            if 'jti' in line.lower() and EMAIL.split('@')[0] in line:
                                # Try to extract jti
                                jti_match = re.search(r'jti["\s]+([a-f0-9-]+)', line, re.IGNORECASE)
                                if jti_match:
        ...
        if '/onboarding' in current_url or '/dashboard' in current_url:
            ...
            await page.goto('http://localhost:5173/app/onboarding', wait_until='networkidle', timeout=60000)
            await asyncio.sleep(3)
            current_url = page.url
            if '/onboarding' in current_url or '/dashboard' in current_url:
                findings['step1_authentication']['authenticated'] = True
        ...
        if '/dashboard' in current_url:
            ...
            await asyncio.sleep(2)
            try:
                start_btn = page.locator('button:has-text("Start"), button:has-text("Continue"), button:has-text("Get Started")').first
            ...
            if os.path.exists(resume_path):
                ...
                if await file_input.is_visible(timeout=5000):
                    ...
                    await page.screenshot(path='/tmp/step2_resume_uploaded.png', full_page=True)
                    findings['screenshots'].append('/tmp/step2_resume_uploaded.png')
                else:
                    print("   ⚠ File upload input not found - trying alternative methods")
                    # Try LinkedIn URL input
                    linkedin_input = page.locator('input[placeholder*="linkedin" i], input[type="url"]').first
            ...
            page_text = await page.inner_text('body')
            skills_to_check = ['python', 'javascript', 'react', 'typescript', 'fastapi', 'postgresql', 'docker', 'aws']
            ...
            for skill in skills_to_check:
                if skill not in found_skills:
                    try:
                        skill_input = page.locator('input[placeholder*="skill" i], input[placeholder*="Add" i]').first
            ...
            # Save and continue
            save_btn = page.locator('button:has-text("Save"), button:has-text("Continue")').first
            ...
            # Fill contact info
            first_name_input = page.locator('input[placeholder*="first" i], input[name*="first" i]').first
            if await first_name_input.is_visible(timeout=3000):
                await first_name_input.fill('John')
                await asyncio.sleep(1)
            
            last_name_input = page.locator('input[placeholder*="last" i], input[name*="last" i]').first
            if await last_name_input.is_visible(timeout=3000):
                await last_name_input.fill('Doe')
                await asyncio.sleep(1)
            
            phone_input = page.locator('input[type="tel"], input[placeholder*="phone" i]').first
            ...
            location_input = page.locator('input[placeholder*="location" i], input[placeholder*="city" i]').first
            ...
            role_input = page.locator('input[placeholder*="role" i], input[placeholder*="title" i]').first
            if await role_input.is_visible(timeout=3000):
                await role_input.fill('Senior Software Engineer')
                await asyncio.sleep(1)
            
            salary_min_input = page.locator('input[placeholder*="min" i], input[type="number"]').first
            ...
            remote_checkbox = page.locator('input[type="checkbox"][aria-label*="remote" i], label:has-text("Remote")').first
            if await remote_checkbox.is_visible(timeout=3000):
                await remote_checkbox.click()
                await asyncio.sleep(1)
            
            save_prefs_btn = page.locator('button:has-text("Save"), button:has-text("Continue")').first
            ...
            save_workstyle_btn = page.locator('button:has-text("Save"), button:has-text("Continue")').first
            ...
            if await goals_textarea.is_visible(timeout=3000):
                await goals_textarea.fill('Looking for senior engineering roles in fast-growing tech companies where I can lead technical initiatives and build scalable systems')
                await asyncio.sleep(1)
            ...
            complete_btn = page.locator('button:has-text("Complete"), button:has-text("Finish")').first
        ...
        await page.goto('http://localhost:5173/app/dashboard', wait_until='networkidle', timeout=60000)
        await asyncio.sleep(5)
        ...
        start_time = time.time()
        await page.goto('http://localhost:5173/app/jobs', wait_until='networkidle', timeout=60000)
        load_time = time.time() - start_time
        ...
        job_elements = await page.locator('article[role="article"], [role="article"]').all()
        ...
        if match_scores:
            ...
            try:
                ...
                if score_elements:
                    print(f"   Found {len(score_elements)} potential match score elements")
            except:
                pass
        ...
        if job_count > 0:
            ...
            if await location_input.is_visible(timeout=3000):
                ...
                await page.screenshot(path='/tmp/step4_location_filter.png', full_page=True)
                findings['screenshots'].append('/tmp/step4_location_filter.png')
            
            # Salary filter
            print("   Testing Salary Filter...")
            salary_input = page.locator('input[placeholder*="salary" i], input[type="number"]').first
            ...
            remote_checkbox = page.locator('input[type="checkbox"][aria-label*="remote" i]').first
            if await remote_checkbox.is_visible(timeout=3000):
                await remote_checkbox.click()
                await asyncio.sleep(3)
            ...
            search_input = page.locator('input[placeholder*="search" i], input[placeholder*="keyword" i]').first
        ...
        if job_count > 0:
            ...
            try:
                ...
                if 'match' in page_text.lower() and ('why' in page_text.lower() or 'explanation' in page_text.lower()):
        ...
        if job_count > 0:
            ...
            apply_btn = page.locator('button:has-text("Apply"), button:has-text("Swipe")').first
            ...
            await page.goto('http://localhost:5173/app/applications', wait_until='networkidle', timeout=60000)
            await asyncio.sleep(5)
        ...
        successful_jobs_calls = [r for r in findings['network_responses'] if 'jobs' in r['url'] and r['status'] == 200]
        findings['step7_verification']['jobs_api_calls'] = len(jobs_api_calls)
        findings['step7_verification']['successful_jobs_calls'] = len(successful_jobs_calls)
        ...
        print(f"  Authenticated: {findings['step1_authentication'].get('authenticated', False)}")
        
        print(f"\nStep 2 - Onboarding:")
        print(f"  Complete: {findings['step2_onboarding'].get('onboarding_complete', False)}")
        print(f"  Resume Parsed: {findings['step2_onboarding'].get('resume_parsed', False)}")
        print(f"  Skills Extracted: {len(findings['step2_onboarding'].get('skills_extracted', []))}")
        
        print(f"\nStep 3 - Dashboard:")
        print(f"  Loaded: {findings['step3_dashboard'].get('dashboard_loaded', False)}")
        
        print(f"\nStep 4 - Job Matching:")
        print(f"  Jobs Displayed: {findings['step4_job_matching'].get('jobs_displayed', False)}")
        print(f"  Job Count: {findings['step4_job_matching'].get('job_count', 0)}")
        print(f"  Match Scores Found: {findings['step4_job_matching'].get('match_scores_found', False)}")
        if findings['step4_job_matching'].get('match_scores'):
            print(f"  Match Scores: {findings['step4_job_matching']['match_scores']}")
        
        print(f"\nStep 5 - Job Details:")
        print(f"  Job Details Opened: {findings['step5_job_details'].get('job_details_opened', False)}")
        print(f"  Match Explanation: {findings['step5_job_details'].get('match_explanation_found', False)}")
        
        print(f"\nStep 6 - Application:")
        print(f"  Application Submitted: {findings['step6_application'].get('application_submitted', False)}")
        
        print(f"\nStep 7 - Verification:")
        print(f"  Console Errors: {findings['step7_verification'].get('console_errors_count', 0)}")
        print(f"  Successful API Calls: {findings['step7_verification'].get('successful_jobs_calls', 0)}")
