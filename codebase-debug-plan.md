...
|------|----------|-------------|---------|--------|--------------|
| **ruff** | Static Analysis | `pip install ruff` | `ruff check apps packages shared --select E,W,F,I` | ✅ Installed | 403 errors: F821, F841, W291, W293, I001, F811 |
| **mypy** | Type Checking | `pip install mypy` | `mypy apps/api/ apps/worker/ packages/backend/ shared/ --ignore-missing-imports` | ✅ Installed | 351+ errors: attr-defined, assignment, union-attr, no-any-return |
| **bandit** | Security | `pip install bandit` | `bandit -c pyproject.toml -r apps packages shared` | ✅ Installed | 2 B110 try_except_pass in apps/api/sessions.py |
...
| **pytest-cov** | Coverage | `pip install pytest-cov` | `pytest tests/ --cov=apps --cov=packages --cov-report=html` | ✅ Installed | 5 collection errors, 2 failed, ~2% coverage |
...
| **hadolint** | Docker | `npm install` | `docker run hadolint/hadolint hadolint Dockerfile` | ✅ Installed | DL3008, DL3013, DL3025, DL3059 |
...
| **Syntax: async with** | tests/test_critical_endpoints.py:85, test_onboarding_api.py:78 | Critical | Use `async def` for test functions | pytest |
| **ImportError** | tests/test_failure_drills.py, test_integration.py | Critical | `pip install python-docx`; fix worker import path | pytest |
...
| **Dockerfile** | Dockerfile:22, 62, 83 | Low | Pin apt/pip versions; use JSON CMD | hadolint |
...
1. **Fix test collection errors**
   - `pip install python-docx` (add to requirements.txt if needed)
   - Fix `async with` in tests: wrap in `async def test_...` and use `@pytest.mark.asyncio`
...
|--------|--------|---------|--------|
| Ruff critical (E,F) | 0 | ~10 | ❌ |
| Mypy errors | 0 | 351+ | ❌ |
| Bandit high/critical | 0 | 0 | ✅ |
| pip-audit vulns | 0 | 0 | ✅ |
| Test collection | 0 errors | 5 collection errors | ❌ |
| Test pass rate | 100% | 50 passed, 2 failed | ⚠️ |
| Coverage | > 80% | ~2% | ❌ |
| TypeScript errors | 0 | 9+ | ❌ |
| ESLint | 0 | Plugin missing | ⚠️ |
| Hadolint | 0 | 3 warnings | ⚠️ |
