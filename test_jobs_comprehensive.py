...
async def test_jobs_comprehensive():
    async with async_playwright() as p:
        ...
        def track_response(resp):
            if 'localhost:8000' in resp.url and ('jobs' in resp.url or 'profile' in resp.url):
        ...
        if '/login' in current_url:
            ...
            if '/login' in current_url:
                print("   ❌ Still on login page - cannot proceed without authentication")
                await page.screenshot(path='/tmp/jobs_login_required.png', full_page=True)
            ...
            try:
                ...
                if len(elements) > 0:
                    ...
                    for elem in elements:
                        ...
                        if any(keyword in text.lower() for keyword in ['engineer', 'developer', 'software', 'company', 'salary', 'location', 'remote', 'apply']):
        ...
        job_titles = await page.locator('h2, h3, [class*="title"], [class*="Title"]').all()
        potential_jobs = []
        for title in job_titles:
            text = await title.text_content() or ''
            if any(keyword in text.lower() for keyword in ['engineer', 'developer', 'software', 'manager', 'analyst']):
        ...
        if job_elements:
            try:
                ...
                if 'detail' in page_text_after.lower() or 'description' in page_text_after.lower() or current_url_after != current_url:
        ...
        successful_jobs_calls = [r for r in findings['api_responses'] if 'jobs' in r['url'] and r['status'] == 200]
