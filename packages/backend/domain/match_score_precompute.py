...
from packages.backend.domain.semantic_matching import SemanticMatchingService, get_matching_service
...
async def precompute_match_scores(
    ...
    try:
        async with db_pool.acquire() as conn:
            ...
            if user_id:
                user_query = "SELECT id FROM public.users WHERE id = $1 AND is_active = true"
            ...
            if not user_rows or not job_rows:
                logger.info("No users or jobs to process for match score pre-computation")
                return stats
            ...
            observe("match_score.precompute.duration", 0)  # Would measure actual duration
            incr("match_score.precompute.completed", {
    ...
    async def handler(job_data: dict[str, Any]) -> JobResult:
        ...
        try:
            ...
            if not db_pool:
                return JobResult(
                    success=False,
                    error="Database pool not available in job queue"
                )
