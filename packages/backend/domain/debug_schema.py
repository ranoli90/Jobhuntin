...
async def debug_alter_stmts(conn: asyncpg.Connection, log_lines: list[str]) -> None:
    alter_stmts = [
        ...
        "CREATE OR REPLACE FUNCTION public.claim_next_prioritized("
            "p_max_attempts int DEFAULT 3) RETURNS SETOF public.applications AS $$ "
            "BEGIN RETURN QUERY UPDATE public.applications SET status = 'PROCESSING', "
            "locked_at = now(), updated_at = now() WHERE id = ( "
            "SELECT id FROM public.applications WHERE "
            "(status = 'QUEUED' OR (status = 'PROCESSING' AND locked_at < now() - interval '10 minutes')) "
            "AND (snoozed_until IS NULL OR snoozed_until < now()) "
            "AND (available_at IS NULL OR available_at <= now()) "
            "AND attempt_count < p_max_attempts ORDER BY priority_score DESC, created_at ASC "
