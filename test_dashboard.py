...
async def test_dashboard():
    async with async_playwright() as p:
            ...
            if auth_cookie:
                ...
                with urllib.request.urlopen(req) as response:
                    ...
                    if not has_completed:
                        print("  ⚠ has_completed_onboarding is false - attempting to update via browser...")
                        ...
                        try:
                            ...
                            if result.get('success'):
                                print(f"  ✓ Updated has_completed_onboarding via browser: {result.get('data', {}).get('has_completed_onboarding', False)}")
                                await asyncio.sleep(2)
                                # Refresh to get updated state
                                await page.reload()
                                await asyncio.sleep(3)
                            else:
                                print(f"  ⚠ Browser update failed: {result.get('error', 'Unknown error')[:100]}")
        ...
        await page.goto('http://localhost:5173/app/dashboard', wait_until='networkidle', timeout=30000)
        ...
        if '/login' in current_url:
            ...
            if '/onboarding' in current_url:
                print("  Still on onboarding - checking if we can access dashboard content")
        ...
        # Test navigation
        print("\n  Testing Navigation...")
        nav_links = await page.locator('nav a, [role="navigation"] a, a[href*="/app"]').all()
        ...
        # Test job search
        print("\n  Testing Job Search...")
        search_input = page.locator('input[placeholder*="search" i], input[placeholder*="job" i]').first
        ...
        # Test profile
        print("\n  Testing Profile...")
        profile_link = page.locator('a:has-text("Profile"), a[href*="profile"], button:has-text("Profile")').first
        ...
        apps_link = page.locator('a:has-text("Application"), a[href*="application"], button:has-text("Application")').first
        ...
        # Test matches
        print("\n  Testing Matches...")
        matches_link = page.locator('a:has-text("Match"), a[href*="match"], button:has-text("Match")').first
