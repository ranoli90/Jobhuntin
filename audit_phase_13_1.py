...
def audit_phase_13_1():
    ...
    try:
        ...
        if os.path.exists("packages/backend/domain/email_communication_manager.py"):
            with open(
                "packages/backend/domain/email_communication_manager.py", "r"
            ) as f:
                content = f.read()
            ...
            for method in required_methods:
                if f"def {method}" in content:
                    ...
                    issues_found.append(
                        f"EmailCommunicationManager missing method: {method}"
                    )
        ...
        if os.path.exists("packages/backend/domain/semantic_notification_matcher.py"):
            with open(
                "packages/backend/domain/semantic_notification_matcher.py", "r"
            ) as f:
                content = f.read()
            ...
            for method in required_methods:
                if f"def {method}" in content:
                    ...
                    print(
                        f"FAILED: SemanticNotificationMatcher missing method: {method}"
                    )
                    issues_found.append(
                        f"SemanticNotificationMatcher missing method: {method}"
                    )
