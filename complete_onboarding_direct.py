...
async def complete_onboarding():
    async with async_playwright() as p:
        ...
        print("Navigating to verify-magic endpoint...")
        ...
        start_btn = page.locator(
            'button:has-text("Start setup"), button:has-text("Get Started"), button:has-text("Start Setup")'
        ).first
            ...
            try:
                if await inp.is_visible(timeout=1000):
                    ...
                    if "skill" in placeholder.lower() or "skill" in name.lower():
                        skills_input = inp
                        print(
                            f"  Found skills input: placeholder='{placeholder}', name='{name}', type='{input_type}'"
                        )
                        break
            except:
                continue
        ...
        try:
            first_label = page.locator(
                'text=/first.*name/i, label:has-text("First")'
            ).first
            if await first_label.is_visible(timeout=2000):
                # Find associated input
                first_input = first_label.locator(
                    "xpath=following::input[1] | ../input"
                ).first
        ...
        try:
            last_label = page.locator(
                'text=/last.*name/i, label:has-text("Last")'
            ).first
            if await last_label.is_visible(timeout=2000):
                last_input = last_label.locator(
                    "xpath=following::input[1] | ../input"
                ).first
            ...
            try:
                ...
                for inp in inputs:
                    if await inp.is_visible(timeout=2000):
                        placeholder = await inp.get_attribute("placeholder") or ""
                        if (
                            "location" in placeholder.lower()
                            or "remote" in placeholder.lower()
                            or "san francisco" in placeholder.lower()
                        ):
        ...
        for selector in [
            ...
            'input[placeholder*="Product Manager" i]',
            'input[role="combobox"]',
        ]:
            try:
                inputs = await page.locator(selector).all()
                for inp in inputs:
                    if await inp.is_visible(timeout=2000):
                        placeholder = await inp.get_attribute("placeholder") or ""
                        if (
                            "role" in placeholder.lower()
                            or "manager" in placeholder.lower()
                        ):
        ...
        for selector in [
            ...
            try:
                ...
                if await btn.is_visible(timeout=3000):
                    ...
                    if (
                        "resume" in page_text_after.lower()
                        or "upload" in page_text_after.lower()
                    ):
                        print("  ✓ Preferences step completed - moved to Resume step")
                    else:
                        print(
                            f"  ⚠ Step transition unclear, page shows: {page_text_after[:100]}..."
                        )
                    break
            except:
                continue
        ...
        print(
            f"  Filled {filled_count} text fields, selected {selected_count} radio options, checked {checked_count} checkboxes"
        )
        ...
        try:
            ...
            goal_label = page.locator(
                'text=/career.*goal/i, label:has-text("goal")'
            ).first
            if await goal_label.is_visible(timeout=2000):
                goal_textarea = goal_label.locator(
                    "xpath=following::textarea[1] | ../textarea | following::input[1]"
                ).first
                if await goal_textarea.is_visible(timeout=2000):
                    await goal_textarea.fill(
