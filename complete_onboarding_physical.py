...
async def complete_onboarding():
    async with async_playwright() as p:
        ...
        for selector in welcome_selectors:
            try:
                ...
                if await btn.is_visible(timeout=3000):
                    ...
                    if 'accept' in text.lower() or 'reject' in text.lower() or 'manage' in text.lower():
                        continue
            ...
            try:
                ...
                for btn in all_buttons:
                    if await btn.is_visible(timeout=2000):
                        ...
                        if text and 'accept' not in text.lower() and 'reject' not in text.lower() and 'manage' not in text.lower() and 'restart' not in text.lower():
                            if 'start' in text.lower() or 'setup' in text.lower() or 'continue' in text.lower() or 'next' in text.lower():
        ...
        if 'Location' in page_text and 'Role type' in page_text:
            print("  ⚠ Detected Preferences step instead of Resume step - may have skipped ahead")
        elif 'resume' in page_text.lower() or 'upload' in page_text.lower() or 'linkedin' in page_text.lower():
        ...
        all_elements = await page.locator('input, textarea, button, label, div[role="textbox"]').all()
        ...
        for i, elem in enumerate(all_elements):
            try:
                ...
                if visible:
                    ...
                    print(f"    {tag} {i}: type='{input_type}', role='{role}', name='{name}', id='{id_attr}', placeholder='{placeholder[:30]}', text='{text[:30]}'")
            except:
                pass
        ...
        if not linkedin_filled:
            for elem in visible_inputs:
                try:
                    ...
                    if tag == 'INPUT' and (input_type == 'url' or 'linkedin' in placeholder or 'url' in placeholder):
                        await elem.fill("https://linkedin.com/in/johndoe")
                        print(f"  ✓ Filled LinkedIn URL by checking input attributes")
                        linkedin_filled = True
                        break
                except:
                    continue
        ...
        if 'resume' in page_text_check.lower() or 'upload' in page_text_check.lower() or 'linkedin' in page_text_check.lower():
        ...
        page_text_check = await page.inner_text('body')
        if 'skill' in page_text_check.lower() and ('review' in page_text_check.lower() or 'add' in page_text_check.lower()):
                    ...
                    for inp in inputs:
                        if await inp.is_visible(timeout=2000):
                            ...
                            if 'skill' in placeholder.lower():
                                skills_input = inp
                                print(f"  Found skills input using: {selector}, placeholder: '{placeholder}'")
                                break
                    if skills_input:
                        break
                except:
            ...
            if skills_input:
                for skill in skills:
                    try:
                        ...
                        add_btns = await page.locator('button:has-text("Add"), button[type="submit"], button:has-text("+")').all()
                ...
                print("  ⚠ Could not find skills input field - skills may already be loaded from resume")
        ...
        for selector in ['input[name*="location" i]', 'input[placeholder*="location" i]', 'input[placeholder*="Location" i]']:
            try:
                inp = page.locator(selector).first
                if await inp.is_visible(timeout=2000):
                    ...
                    dropdown_option = page.locator('div[role="option"], li[role="option"]').first
        ...
        role_filled = False
        for selector in ['input[name*="role" i]', 'input[name*="title" i]', 'input[placeholder*="role" i]', 'input[placeholder*="title" i]']:
            try:
                inp = page.locator(selector).first
                if await inp.is_visible(timeout=2000):
        ...
        page_text_check = await page.inner_text('body')
        if 'work style' in page_text_check.lower() or 'workstyle' in page_text_check.lower():
            ...
            continue_btn = page.locator('button[data-onboarding-next], button:has-text("Save"), button:has-text("Continue"):not(:has-text("Restart"))').first
            ...
            print(f"  ⚠ Not on Work Style step")
