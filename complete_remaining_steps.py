...
async def complete_remaining_steps():
    async with async_playwright() as p:
        ...
        async def click_continue():
            for selector in ['button:has-text("Continue")', 'button:has-text("Next")', 'button:has-text("Save & Continue")']:
                try:
                    btn = page.locator(selector).first
                    if await btn.is_visible(timeout=2000):
                        text = await btn.text_content()
                        if 'preferences' not in text.lower() or 'save preferences' in text.lower():
        ...
        if 'work style' in page_text.lower() or 'workstyle' in page_text.lower():
            ...
            for inp in text_inputs:
                try:
                    if await inp.is_visible(timeout=2000):
                        ...
                        if value or 'email' in placeholder.lower() or 'phone' in placeholder.lower() or 'location' in placeholder.lower() or 'name' in placeholder.lower():
                            continue
                        await inp.fill("I prefer collaborative environments with clear communication and regular feedback")
                        filled += 1
                        print(f"  ✓ Filled text field: '{placeholder[:30]}' or '{name[:30]}'")
                except:
                    pass
            ...
            # Click Continue
            for selector in ['button:has-text("Continue")', 'button:has-text("Next")', 'button:has-text("Save")']:
                try:
                    btn = page.locator(selector).first
                ...
                try:
                    if await textarea.is_visible(timeout=2000):
                        ...
                        if not value:  # Only fill if empty
                            ...
                            print(f"  ✓ Filled textarea: '{placeholder[:30]}' or '{name[:30]}'")
                            goals_filled = True
                            break
                except:
                    continue
            ...
            for inp in text_inputs:
                try:
                    if await inp.is_visible(timeout=2000):
                        ...
                        if value or 'email' in placeholder.lower() or 'phone' in placeholder.lower() or 'name' in placeholder.lower():
                            continue
                ...
                for btn in all_buttons:
                    try:
                        if await btn.is_visible(timeout=1000):
                            ...
                            if text and 'back' not in text.lower() and 'restart' not in text.lower() and len(text.strip()) > 0:
                                if 'complete' in text.lower() or 'finish' in text.lower() or 'ready' in text.lower() or 'start' not in text.lower():
