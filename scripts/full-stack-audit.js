 ...
 * Runs: TypeScript, ESLint, depcheck, knip, npm audit (web);
 *       ruff, mypy, bandit, pip-audit, vulture, radon (Python).
...
function main() {
  ...
  const pyPaths = ['apps/api', 'apps/worker', 'packages/backend', 'shared', 'scripts'].join(' ');
  if (run('Ruff (lint)', 'python', ['-m', 'ruff', 'check', 'apps', 'packages', 'shared', 'scripts', '--select', 'E,W,F,I', '--output-format', 'concise']) !== 0) hasErrors = true;
  if (run('Mypy (types)', 'python', ['-m', 'mypy', 'apps/api/', 'apps/worker/', 'packages/backend/', 'shared/', '--ignore-missing-imports', '--no-error-summary'], ROOT, { PYTHONPATH: 'apps:packages:.' }) !== 0) hasErrors = true;
  if (run('Bandit (security)', 'python', ['-m', 'bandit', '-r', 'apps', 'packages', 'shared', 'scripts', '-x', '.venv,node_modules,__pycache__', '-f', 'txt', '-q']) !== 0) hasErrors = true;
  if (run('pip-audit (vulns)', 'python', ['-m', 'pip_audit', '-r', 'requirements.txt', '-r', 'requirements-dev.txt']) !== 0) hasErrors = true;
  if (run('Vulture (dead code)', 'python', ['-m', 'vulture', 'apps', 'packages', 'shared', 'scripts', '--min-confidence', '80']) !== 0) hasErrors = true;
  if (run('Radon (complexity)', 'python', ['-m', 'radon', 'cc', 'apps', 'packages', 'shared', '-a', '-s', '--total-average']) !== 0) hasErrors = true;
