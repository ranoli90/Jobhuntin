...
async def check_database() -> CheckResult:
    ...
    try:
        ...
        try:
            ...
            return CheckResult(
                "Database Tables", True, f"{len(found)} tables found", ms
            )
        finally:
            await conn.close()
    except Exception as exc:
    ...
    try:
        ...
        try:
            ...
            if count and count > 0:
                return CheckResult(
                    "Job Feed Data", True, f"{count} jobs in database", ms
                )
            return CheckResult(
                "Job Feed Data", False, "No jobs found — run seed_beta.py", ms
            )
        finally:
            await conn.close()
    except Exception as exc:
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=10) as client:
            ...
            if resp.status_code == 200:
                body = resp.json()
                return CheckResult(
                    "Analytics Sink", True, f"accepted={body.get('accepted', '?')}", ms
                )
            return CheckResult(
                "Analytics Sink", False, f"status={resp.status_code}", ms
            )
    except Exception as exc:
        ms = int((time.monotonic() - t0) * 1000)
        ...
        async with httpx.AsyncClient(timeout=10) as client:
            ...
            if resp.status_code in (200, 401, 403, 422):
                return CheckResult(
                    "Billing Endpoint",
                    True,
                    f"status={resp.status_code} (expected auth-gated)",
                    ms,
                )
            return CheckResult(
                "Billing Endpoint", False, f"status={resp.status_code}", ms
            )
    except Exception as exc:
        ms = int((time.monotonic() - t0) * 1000)
    ...
    try:
        async with httpx.AsyncClient(timeout=10) as client:
            ...
            if resp.status_code in (200, 401, 403):
                return CheckResult(
                    "M1 Dashboard", True, f"status={resp.status_code}", ms
                )
...
async def check_worker_recent_activity() -> CheckResult:
    ...
    try:
        ...
        try:
            ...
            if count and count > 0:
                return CheckResult(
                    "Worker Activity", True, f"{count} events in last hour", ms
                )
            return CheckResult(
                "Worker Activity", True, "No recent activity (OK if idle)", ms
            )
        finally:
            await conn.close()
    except Exception as exc:
...
def main() -> None:
    ...
    parser.add_argument(
        "--db", action="store_true", default=True, help="Include database checks"
    )
