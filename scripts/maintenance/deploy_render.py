    ...
    try:
        ...
        if api_service:
            ...
            deploy_resp = httpx.post(
                f"https://api.render.com/v1/services/{api_service['id']}/deploys",
                headers=headers,
                json={},
            )
            if deploy_resp.status_code in {200, 201, 202}:
                ...
                print(
                    f"Failed to trigger backend deploy: {deploy_resp.status_code} - {deploy_resp.text}"
                )
        else:
        ...
        if web_service:
            print(
                f"Found frontend service: {web_service['name']} ({web_service['id']})"
            )

            # Ensure VITE_API_URL is set in web_service env vars
            env_resp = httpx.get(
                f"https://api.render.com/v1/services/{web_service['id']}/env-vars",
                headers=headers,
            )
            ...
            if not has_api_url and api_service:
                ...
                if patch_resp.status_code in {200, 201, 204}:
                    ...
                    print(
                        f"Failed to set VITE_API_URL: {patch_resp.status_code} - {patch_resp.text}"
                    )

            # Trigger deploy for frontend
            print(f"Triggering deploy for {web_service['name']}...")
            deploy_resp = httpx.post(
                f"https://api.render.com/v1/services/{web_service['id']}/deploys",
                headers=headers,
                json={},
            )
            if deploy_resp.status_code in {200, 201, 202}:
                ...
                print(
                    f"Failed to trigger frontend deploy: {deploy_resp.status_code} - {deploy_resp.text}"
                )
        else:
            print(
