...
class EndToEndIntegrationTester:
    ...
    def test_cross_phase_integration(self):
        ...
        tests = [
            ...
            ("Communication to UX Integration", self.test_communication_to_ux_integration),
            ("Database to API Integration", self.test_database_to_api_integration),
            ("Frontend to Backend Integration", self.test_frontend_to_backend_integration),
        ]
    ...
    def test_core_tables_access(self) -> bool:
        ...
        core_tables = [
            "tenants", "users", "jobs", "applications", "events",
            "user_preferences", "tenant_members", "application_inputs"
        ]
            ...
            for migration in migrations_dir.glob("*.sql"):
                try:
                    with open(migration, 'r') as f:
                        ...
                        if (f"CREATE TABLE.*{table}" in content or 
                            f"CREATE TABLE IF NOT EXISTS.*{table}" in content or
                            f"{table}" in content):
                            found = True
                            break
                except:
        ...
        try:
            with open("apps/api/main.py", 'r') as f:
                ...
                return ("ai_mod" in content and 
                        ("agent_improvements_mod" in content or 
                         "agent_improvements" in content.lower()))
        except:
            return False
    ...
    def test_communication_to_ux_integration(self) -> bool:
        ...
        try:
            with open("apps/api/main.py", 'r') as f:
                content = f.read()
                return "communication_endpoints" in content and "user_experience_endpoints" in content
        except:
            return False
