...
async def full_e2e_test():
    async with async_playwright() as p:
        ...
        if await email_input.is_visible(timeout=5000):
            ...
            submit_btn = page.locator('button[type="submit"], button:has-text("Send"), button:has-text("Request")').first
        ...
        try:
            with open('/tmp/backend.log', 'r') as f:
                ...
                for line in reversed(lines[-100:]):  # Check last 100 lines
                    if 'magic link' in line.lower() or 'login?token=' in line.lower() or '/auth/verify-magic' in line.lower():
                        ...
                        for pattern in url_patterns:
                            ...
                            if url_match:
                                ...
                                print(f"   ✓ Found magic link in logs")
        ...
        if '/onboarding' in current_url:
            ...
            if await file_input.is_visible(timeout=5000):
                ...
                skip_btn = page.locator('button:has-text("Skip"), button:has-text("Skip for now")').first
            ...
            # Add skills
            skills_to_add = ['Python', 'JavaScript', 'React', 'TypeScript', 'FastAPI', 'PostgreSQL', 'Docker', 'AWS']
        ...
        await page.screenshot(path='/tmp/phase1_onboarding_complete.png', full_page=True)
        ...
        if len(job_elements) > 0:
            ...
            try:
                ...
                await page.screenshot(path='/tmp/phase4_job_details.png', full_page=True)
        ...
        print(f"\nPhase 1 - Onboarding:")
        print(f"  Complete: {findings['phase1_onboarding'].get('onboarding_complete', False)}")
        
        print(f"\nPhase 2 - Dashboard:")
        print(f"  Loaded: {findings['phase2_dashboard'].get('dashboard_loaded', False)}")
        
        print(f"\nPhase 3 - Job Matching:")
        print(f"  Jobs Displayed: {findings['phase3_job_matching'].get('jobs_displayed', False)}")
        print(f"  Job Count: {findings['phase3_job_matching'].get('job_count', 0)}")
        print(f"  Match Scores Found: {findings['phase3_job_matching'].get('match_scores_found', False)}")
        if findings['phase3_job_matching'].get('match_scores'):
            print(f"  Match Scores: {findings['phase3_job_matching']['match_scores']}")
        
        print(f"\nPhase 4 - Job Details:")
        print(f"  Job Details Opened: {findings['phase4_job_details'].get('job_details_opened', False)}")
        
        print(f"\nPhase 5 - Verification:")
        print(f"  Console Errors: {findings['phase5_verification'].get('console_errors_count', 0)}")
        print(f"  Jobs API Calls: {findings['phase5_verification'].get('jobs_api_calls', 0)}")
