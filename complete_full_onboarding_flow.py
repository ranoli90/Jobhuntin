...
async def complete_full_flow():
    async with async_playwright() as p:
        ...
        if current_step == 1 or ('welcome' in page_text.lower() and 'start setup' in page_text.lower()):
            start_btn = page.locator('button:has-text("Start setup"), button:has-text("Get Started")').first
        ...
        if current_step == 2 or current_step == 3 or ('resume' in page_text.lower() and 'upload' in page_text.lower()):
        ...
        if 'skill' in page_text.lower() and ('review' in page_text.lower() or 'add' in page_text.lower()):
            ...
            # Click "Save & Continue"
            await asyncio.sleep(3)
            save_continue_btn = page.locator('button:has-text("Save & Continue"), button:has-text("Continue")').first
        ...
        page_text = await page.inner_text('body')
        if 'contact' in page_text.lower() or 'first name' in page_text.lower() or current_step == 4 or current_step == 5:
            print("  ✓ On Contact step")
            
            # Fill First Name
            first_name_input = page.locator('input[name*="first" i], input[placeholder*="first" i]').first
            ...
            # Fill Last Name
            last_name_input = page.locator('input[name*="last" i], input[placeholder*="last" i]').first
        ...
        page_text = await page.inner_text('body')
        if 'preferences' in page_text.lower() or 'location' in page_text.lower() or current_step == 2:
            ...
            save_btn = page.locator('button:has-text("Save preferences"), button:has-text("Continue")').first
        ...
        page_text = await page.inner_text('body')
        if 'work style' in page_text.lower() or 'workstyle' in page_text.lower() or current_step == 6:
        ...
        page_text = await page.inner_text('body')
        if 'career' in page_text.lower() or 'goal' in page_text.lower() or current_step == 7:
        ...
        page_text = await page.inner_text('body')
        if 'ready' in page_text.lower() or 'complete' in page_text.lower() or 'finish' in page_text.lower() or current_step == 8:
            print("  ✓ On Ready step")
        ...
        if '/app/dashboard' in final_url:
            ...
            if 'job' in page_text.lower() or 'application' in page_text.lower() or 'dashboard' in page_text.lower():
                print("  ✓ Dashboard content loaded")
            
            # Try to browse jobs
            try:
                jobs_link = page.locator('a:has-text("Jobs"), button:has-text("Browse Jobs"), a[href*="jobs"]').first
        ...
        try:
            ...
            auth_cookie = next((c for c in cookies if c['name'] == 'jobhuntin_auth'), None)
