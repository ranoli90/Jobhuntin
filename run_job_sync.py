    ...
    for k, v in dict(row).items():
        try:
            ...
            if cleaned is not None:
                raw_data[k] = str(cleaned) if not isinstance(cleaned, (list, dict)) else cleaned
        except:
            pass
            ...
            for _, row in df.iterrows():
                ...
                if existing:
                    await conn.execute(
                        ...
                        job["location"], job["is_remote"], job["job_type"],
                        job["salary_min"], job["salary_max"],
                        job["application_url"], job["source"],
                        job["date_posted"], job["job_level"],
                        job["company_industry"], job["company_logo_url"],
                    ...
                    await conn.execute(
                        ...
                        job["description"], job["location"], job["is_remote"],
                        job["job_type"], job["salary_min"], job["salary_max"],
                        job["application_url"], job["source"], job["date_posted"],
                        job["job_level"], job["company_industry"],
                        job["company_logo_url"], json.dumps(job["raw_data"]),
                    )
                    total_new += 1
            
            print(f"  Running totals: {total_new} new, {total_updated} updated, {total_skipped} skipped")
