...
async def complete_final_steps():
    async with async_playwright() as p:
        ...
        async def get_step_number():
            try:
                step_elem = page.locator('text=/STEP \\d+ OF \\d+/i, text=/Step \\d+ of \\d+/i').first
        ...
        if current_step == 6 or 'work style' in page_text.lower():
            ...
            await asyncio.sleep(2)
            save_btn = page.locator('button:has-text("Save Work Style"), button:has-text("Save work style"), button:has-text("Continue")').first
                    ...
                    try:
                        if await elem.is_visible(timeout=3000):
                            ...
                            if not text or len(text) < 10:
                                await elem.fill("Looking for senior engineering roles in fast-growing tech companies where I can lead technical initiatives and mentor junior developers")
                ...
                for inp in all_inputs:
                    try:
                        if await inp.is_visible(timeout=2000):
                            ...
                            value = await inp.input_value()
                            print(f"    Input: tag={tag}, placeholder='{placeholder[:30]}', value='{value[:30]}'")
                            if 'goal' in placeholder.lower() or (tag.lower() == 'textarea' and not value):
                                await inp.fill("Looking for senior engineering roles in fast-growing tech companies where I can lead technical initiatives and mentor junior developers")
