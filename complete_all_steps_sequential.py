...
async def complete_all_steps():
    async with async_playwright() as p:
        ...
        page.on("console", lambda msg: (
            ...
            console_messages.append(f"{msg.type}: {msg.text}") if msg.type in ["error", "warning"] else None
        ))
        ...
        async def get_step_number():
            try:
                ...
                for selector in selectors:
                    try:
                        ...
                        if await step_elem.is_visible(timeout=2000):
                            ...
                            match = re.search(r'(?:STEP|Step)\s+(\d+)', text, re.IGNORECASE)
                            if match:
                                step_num = int(match.group(1))
                                print(f"    [DEBUG] Found step {step_num} using selector: {selector}")
                                return step_num
                    except:
                        continue
                ...
                import re
                match = re.search(r'(?:STEP|Step)\s+(\d+)\s+(?:of|OF)', page_text, re.IGNORECASE)
                if match:
                    step_num = int(match.group(1))
            ...
            if await start_btn.is_visible(timeout=5000):
                ...
                if not disabled:
                    ...
                    try:
                        ...
                        await page.evaluate('document.querySelector("button:has-text(\'Start setup\')")?.click()')
                        await asyncio.sleep(2)
        ...
        if current_step == 4 or ('skill' in page_text.lower() and 'review' in page_text.lower()):
        ...
        if current_step == 5 or 'contact' in page_text.lower() or 'first name' in page_text.lower():
