...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("schema_verification.log")],
)
...
class DatabaseSchemaVerifier:
    ...
    def __init__(self) -> None:
        ...
        logger.info(
            "DatabaseSchemaVerifier initialized with {} required tables".format(
        ...
        for table_name, description in self.required_tables.items():
            ...
            if exists:
                ...
                table_results.append(
                    TableVerificationResult(table_name, exists, details.split(":")[0])
                )
            else:
                print(f"X {table_name:<30} - {description} ({details})")
                missing_count += 1
                table_results.append(
                    TableVerificationResult(table_name, exists, None, details)
                )
        ...
        for migration_file in migration_files:
            ...
            try:
                ...
                if "+migrate Up" not in content:
                    issues.append(
                        f"Missing '+migrate Up' directive: {migration_file.name}"
                    )

                if "CREATE TABLE" in content and "IF NOT EXISTS" not in content:
                    issues.append(
                        f"Missing 'IF NOT EXISTS' in table creation: {migration_file.name}"
                    )
