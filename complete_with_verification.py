    ...
    async with async_playwright() as p:
        ...
        if current_step == 5 and len(radios) > 0:
            ...
            continue_btn = page.locator(
                'button:has-text("Continue"), button:has-text("Save work style")'
            ).first
            ...
            print(
                f"  ⚠ Not on Work Style step (step={current_step}, radios={len(radios)})"
            )
        ...
        if current_step == 6 and len(textareas) > 0:
            ...
            print(
                f"  ⚠ Not on Career Goals step (step={current_step}, textareas={len(textareas)})"
            )
