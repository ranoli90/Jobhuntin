...
export function useJobMatchScores() {
  ...
  const profileReference = useRef<Record<string, unknown> | null>(null);
  const dealbreakersReference = useRef<DealbreakerPreferences | null>(null);
  const scoresReference = useRef<ScoreCache>({});
  const loadingReference = useRef<Set<string>>(new Set());

  scoresReference.current = scores;
  loadingReference.current = loading;
  ...
  const setDealbreakers = useCallback(
    (dealbreakers: DealbreakerPreferences) => {
      dealbreakersReference.current = dealbreakers;
    },
    [],
  );
  ...
  const scoreJobs = useCallback(async (jobs: JobPosting[]) => {
    ...
    const currentScores = scoresReference.current;
    const currentLoading = loadingReference.current;

    const unscored = jobs.filter(
      (job) => !currentScores[job.id] && !currentLoading.has(job.id),
    );
    if (unscored.length === 0) return;

    setLoading((previous) => {
      const next = new Set(previous);
      for (const job of unscored) next.add(job.id);
      return next;
    });
    ...
    try {
      ...
      const response = await apiPost<{ results: JobMatchScore[] }>(
        "ai/semantic-match/batch",
        {
          ...
          jobs: jobsPayload,
          dealbreakers: dealbreakersReference.current || {},
        },
      );
      ...
      const message =
        error_ instanceof Error ? error_.message : "Failed to score jobs";
      setError(message);
      pushToast({
      ...
      setLoading((previous) => {
        const next = new Set(previous);
        for (const job of unscored) next.delete(job.id);
        return next;
      });
    }
  }, []);

  const scoreSingleJob = useCallback(
    async (
      job: JobPosting,
      ...
      try {
        return await apiPost<JobMatchScore>("ai/semantic-match", {
          profile,
          jobs: [
            {
              id: job.id,
  ...
  const getScore = useCallback(
    (jobId: string): JobMatchScore | undefined => {
      return scores[jobId];
    },
    [scores],
  );
