...
export function useAISuggestions() {
  ...
  const suggestRoles = useCallback(async (profile: Record<string, unknown>) => {
    ...
    try {
      const result = await apiPost<RoleSuggestion>("ai/suggest-roles", {
        profile,
      });
      ...
      const message =
        error instanceof Error
          ? error.message
          : "Failed to get role suggestions";
   ...
   */
  const suggestSalary = useCallback(
    async (
      ...
      try {
        ...
        const message =
          error instanceof Error
            ? error.message
            : "Failed to get salary suggestion";
   ...
   */
  const suggestLocations = useCallback(
    async (profile: Record<string, unknown>, currentLocation = "") => {
      setLocations({ data: null, loading: true, error: null });
      try {
        const result = await apiPost<LocationSuggestion>(
   ...
   */
  const scoreJobsBatch = useCallback(
    async (
      profile: Record<string, unknown>,
    ...
    async (profile: Record<string, unknown>, currentLocation = "") => {
      ...
      if (rolesResult?.primary_role) {
        salaryResult = await suggestSalary(
          ...
          locationsResult?.suggested_locations?.[0] ||
            currentLocation ||
            "Remote",
        );
      }
