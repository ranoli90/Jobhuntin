...
class SecureStorage {
  ...
  private async initEncryptionKey(): Promise<void> {
    try {
      ...
      if (storedKey) {
        ...
        sessionStorage.setItem('secure_storage_key', JSON.stringify(Array.from(new Uint8Array(exportedKey))));
      }
    } catch (error) {
      console.error('[SecureStorage] Failed to initialize encryption:', error);
      telemetry.track('Secure Storage Error', { error: 'encryption_init_failed' });
    }
  }
  ...
  async getItem<T = any>(key: string): Promise<T | null> {
    try {
      ...
      if (this.isExpired(storedData.timestamp, storedData.ttl)) {
        ...
        telemetry.track('Secure Storage Expired', { key: key.replace(/pii|contact|email|name/gi, '***') });
        return null;
      }
  ...
  clear(): void {
    ...
    keys.forEach(key => {
      if (key.startsWith('secure_') || key.includes('pii') || key.includes('contact')) {
        sessionStorage.removeItem(key);
      }
    });
