...
export const DatabaseMetrics = {
  async getConnectionStats() {
    try {
      ...
      return {
        total: Number.parseInt(total.rows[0].count),
        active: Number.parseInt(active.rows[0].count),
        idle: Number.parseInt(idle.rows[0].count),
        maxConnections: 103,
        utilization: (Number.parseInt(total.rows[0].count) / 103) * 100,
        poolStats: {
...
export const CacheMetrics = {
  async getRedisStats() {
    try {
      ...
      return {
        connected_clients: Number.parseInt(stats.connected_clients || "0"),
        used_memory: stats.used_memory_human,
        used_memory_peak: stats.used_memory_peak_human,
        total_commands_processed: Number.parseInt(
          stats.total_commands_processed || "0",
        ),
        total_connections_received: Number.parseInt(
          stats.total_connections_received || "0",
        ),
        keyspace_hits: Number.parseInt(stats.keyspace_hits || "0"),
        keyspace_misses: Number.parseInt(stats.keyspace_misses || "0"),
        hit_rate:
          stats.keyspace_hits && stats.keyspace_misses
            ? (
                (Number.parseInt(stats.keyspace_hits) /
                  (Number.parseInt(stats.keyspace_hits) +
                    Number.parseInt(stats.keyspace_misses))) *
                100
              ).toFixed(2)
            : "0",
      };
  ...
  async getCacheKeyStats(pattern: string = "*") {
    try {
...
export const HealthMonitor = {
  async getFullHealthStatus() {
    ...
    try {
      const [databaseHealth, redisHealth, databaseStats, redisStats] =
        await Promise.all([
      ...
      return {
        ...
        database: {
          healthy: true,
          stats: databaseStats,
          connections: databaseStats?.poolStats || {},
        },
        redis: {
  ...
  async getPerformanceMetrics() {
    try {
      const [databaseStats, redisStats, slowQueries, tableStats] =
        await Promise.all([
          DatabaseMetrics.getConnectionStats(),
          CacheMetrics.getRedisStats(),
...
export const AlertChecker = {
  async checkAlerts() {
    ...
    if (
      ...
      metrics.database.connections.utilization >
        ALERT_THRESHOLDS.database.connectionUtilization
    ) {
      alerts.push(
        `Database connection utilization high: ${metrics.database.connections.utilization.toFixed(1)}%`,
      );
    }
    ...
    if (
      ...
      if (
        Number.parseFloat(slowest.mean_time) >
        ALERT_THRESHOLDS.database.slowQueryThreshold
      ) {
    ...
    // Redis alerts
    if (
      metrics.redis?.hit_rate &&
      Number.parseFloat(metrics.redis.hit_rate) < ALERT_THRESHOLDS.redis.hitRate
    ) {
    ...
    if (
      ...
      metrics.redis.connected_clients > ALERT_THRESHOLDS.redis.connectionCount
    ) {
      alerts.push(
        `Redis connection count high: ${metrics.redis.connected_clients}`,
      );
    }
    ...
    if (metrics.system?.memory) {
      ...
      if (memoryUtilization > ALERT_THRESHOLDS.system.memoryUtilization) {
        alerts.push(
          `Memory utilization high: ${memoryUtilization.toFixed(1)}%`,
        );
      }
    }
