...
interface AutoCompleteInputProperties extends Omit<
...
export function AutoCompleteInput({
  ...
  const cacheReference = React.useRef<Map<string, string[]>>(new Map());
  ...
  const filteredSuggestions = React.useMemo(() => {
    ...
    if (enableCache) {
      ...
      const cached = cacheReference.current.get(cacheKey);
      if (cached) {
    ...
    if (enableCache) {
      const cacheKey = `${localValue.toLowerCase()}_${suggestions.length}`;
      cacheReference.current.set(cacheKey, filtered);

      // Limit cache size to prevent memory leaks
      if (cacheReference.current.size > 100) {
        const firstKey = cacheReference.current.keys().next().value;
        if (firstKey) {
          cacheReference.current.delete(firstKey);
        }
      }
    }
  ...
  React.useEffect(() => {
    if (highlightedIndex >= 0 && listReference.current) {
      const highlightedElement = listReference.current.children[
        highlightedIndex
      ] as HTMLElement;
