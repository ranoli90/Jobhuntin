...
export interface AISuggestionCardProperties {
...
export function AISuggestionCard({
  ...
  const handleSelect = (suggestion: string) => {
    ...
    if (multiSelect) {
      newSelected = selected.includes(suggestion)
          ...
          return (
            <button
              ...
              className={cn(
                ...
                index === 0 &&
                  !isSelected &&
                  "ring-1 ring-violet-200 ring-offset-2",
              )}
            >
              ...
              {isSelected ? (
                ...
                <svg
                  className="w-4 h-4 text-slate-300 group-hover:text-violet-400 transition-colors"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2"
...
export interface SalarySuggestionCardProperties {
  minSalary: number;
  maxSalary: number;
  marketMedian: number;
  currency?: string;
...
export function SalarySuggestionCard({
  ...
  onReject,
  className,
}: SalarySuggestionCardProperties) {
