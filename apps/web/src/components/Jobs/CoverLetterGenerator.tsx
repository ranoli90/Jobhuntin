...
interface CoverLetterGeneratorProperties {
  job: JobPosting;
  isOpen: boolean;
  onClose: () => void;
}
...
export function CoverLetterGenerator({
  ...
  isOpen,
  onClose,
}: CoverLetterGeneratorProperties) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-4 animate-in fade-in duration-200">
      ...
      <FocusTrap
        ...
        focusTrapOptions={{
          initialFocus: () =>
            cardReference.current?.querySelector<HTMLElement>(
              "button, [href]",
            ) ?? false,
            ...
            {!result && !loading ? (
              <div className="space-y-6">
                <div className="p-4 rounded-xl bg-slate-50 border border-slate-100 text-sm text-slate-600 leading-relaxed">
                  <p>
                    This tool uses your profile and the job description to
                    generate a tailored cover letter. Choose a tone and let AI
