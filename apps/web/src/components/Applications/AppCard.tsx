...
export function AppCard({
  ...
  return (
    <div
      className={cn(
        "rounded-3xl border border-white/70 bg-white px-6 py-5 shadow-sm",
        className,
      )}
    >
      <div className="flex items-start justify-between gap-4">
        ...
        <Badge
          variant="outline"
          className={cn("text-xs", STATUS_TONE[application.status])}
        >
          {STATUS_LABEL[application.status]}
        </Badge>
      </div>
      {application.summary ? (
        <p className="mt-2 text-sm text-brand-ink/70">{application.summary}</p>
      ) : null}
      {application.status === "HOLD" && application.hold_question ? (
        <div className="mt-4 rounded-2xl border border-dashed border-brand-ink/20 bg-brand-shell/60 px-4 py-3 text-sm text-brand-ink">
          ...
          <p className="text-brand-ink/80 text-xs mb-1">
            The application form asked a question we need you to answer:
          </p>
          ...
          {onAnswerHold ? (
            <Button
              ...
              variant="lagoon"
              className="mt-3"
              onClick={() => onAnswerHold(application.id)}
            >
              Answer now
            </Button>
          ) : null}
