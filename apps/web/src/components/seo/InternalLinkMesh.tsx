...
import { ArrowRight, BookOpen, Layers, BarChart3, MapPin, Wrench, Newspaper } from 'lucide-react';
...
const POPULAR_LOCATION_IDS = ['new-york', 'san-francisco', 'seattle', 'austin', 'boston', 'london'];
...
export function InternalLinkMesh({
    ...
    const guidesArray = Object.entries(guidesData as Record<string, { title: string; category?: string }>).map(
        ([slug, data]) => ({ slug, ...data })
    );
    const keywords = [competitorName, ...competitorCategory].map(k => k.toLowerCase()).filter(Boolean);
    const relevantGuides = guidesArray
        .filter(g => keywords.some(kw => (g.title + ' ' + (g.category || '')).toLowerCase().includes(kw)))
        .slice(0, 3);
    const displayGuides = relevantGuides.length > 0 ? relevantGuides : guidesArray.slice(0, 3);

    // Topics: object -> array, pick up to 3 relevant by keywords
    const topicsArray = Object.entries(topicsData as Record<string, { title: string; description?: string }>).map(
        ([slug, data]) => ({ slug, ...data })
    );
    const relevantTopics = topicsArray
        .filter(t => keywords.some(kw => (t.title + ' ' + (t.description || '')).toLowerCase().includes(kw)))
        .slice(0, 3);
    const displayTopics = relevantTopics.length > 0 ? relevantTopics : topicsArray.slice(0, 3);
    ...
    const popularLocations = (locationsData as { id: string; name: string }[])
        ...
        .sort((a, b) => POPULAR_LOCATION_IDS.indexOf(a.id) - POPULAR_LOCATION_IDS.indexOf(b.id))
        .slice(0, 6);
