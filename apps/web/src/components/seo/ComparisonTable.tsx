  ...
  return (
    <div className="overflow-x-auto rounded-3xl border border-slate-200 bg-white shadow-sm">
      <table
        ...
        aria-label={`Feature comparison: JobHuntin vs ${competitor.name}`}
      >
        <thead>
          <tr className="border-b border-slate-100 bg-slate-50/50">
            <th
              scope="col"
              className="px-6 py-5 text-sm font-bold text-slate-500 uppercase tracking-wider"
            >
              Feature
            </th>
            <th
              scope="col"
              className="px-6 py-5 text-sm font-bold text-primary-600 uppercase tracking-wider text-center"
            >
            ...
            <th
              scope="col"
              className="px-6 py-5 text-sm font-bold text-slate-400 uppercase tracking-wider text-center"
            >
              {competitor.name}
            </th>
          </tr>
        ...
        <tbody>
          {features.map((featureKey, index) => (
            <tr
              key={featureKey}
              className={`border-b border-slate-50 ${index % 2 === 0 ? "bg-white" : "bg-slate-50/30"} hover:bg-primary-50/30 transition-colors`}
            >
