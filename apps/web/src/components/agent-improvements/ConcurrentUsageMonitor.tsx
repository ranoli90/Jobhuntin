...
const ConcurrentUsageMonitor: React.FC = () => {
  ...
  const handleFailSession = async (sessionId: string) => {
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to fail session');
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">Real-time monitoring of concurrent application sessions</p>
        </div>
                ...
                <Card key={session.session_id} className="p-4">
                  ...
                  <div className="mt-4">
                    <div className="flex justify-between text-sm mb-2">
                      <span>Progress</span>
                      <span>{session.steps_completed}/{session.total_steps} steps</span>
                    </div>
                  ...
                  {showDetails && (
                    <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                      <div>
                        ...
                        <span className="font-medium">{session.screenshots_captured}</span>
                      </div>
                      <div>
                        <span className="text-gray-500">Buttons:</span>
                        <span className="font-medium">{session.buttons_detected}</span>
                      </div>
                      <div>
                        <span className="text-gray-500">Forms:</span>
                        <span className="font-medium">{session.forms_processed}</span>
                      </div>
                      <div>
                        <span className="text-gray-500">Errors:</span>
                        <span className="font-medium text-red-600">{session.error_count}</span>
                      </div>
                    </div>
                  )}
                </Card>
