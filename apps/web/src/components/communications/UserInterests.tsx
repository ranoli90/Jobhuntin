...
const UserInterests: React.FC = () => {
  ...
  const fetchInteractionHistory = async () => {
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to fetch interaction history');
    }
  };

  const handleAddInterest = async () => {
    try {
      const keywords = interestForm.keywords.split(',').map(k => k.trim()).filter(k => k.length > 0);
  ...
  const handleUpdateInterestScore = async (category: string, scoreAdjustment: number) => {
    try {
      const response = await fetch('/api/communications/interests/update-score', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json',
      ...
      setError(err instanceof Error ? err.message : 'Failed to update interest score');
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">Manage user interests and interaction history</p>
        </div>
            ...
            <CardContent>
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-2xl font-bold">{profile.interaction_history.length}</span>
                </div>
                <div className="flex items-center justify-between">
                  ...
                  <div key={interest.name} className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      ...
                      <div className="flex-1">
                        <h4 className="font-medium capitalize">{interest.name}</h4>
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <Select value={interestForm.category} onValueChange={(value) => setInterestForm({ ...interestForm, category: value })}>
                  <SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setInterestForm({ ...interestForm, keywords: e.target.value })}
                />
              </div>

              <div className="space-y-2">
                <Label htmlFor="score-adjustment">Score Adjustment</Label>
                <Select value={interestForm.score_adjustment.toString()} onValueChange={(value) => setInterestForm({ ...interestForm, score_adjustment: parseFloat(value) })}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div className="flex space-x-2">
                <Button onClick={handleAddInterest} disabled={!interestForm.category || !interestForm.keywords}>
                  <Plus className="h-4 w-4 mr-2" />
                  Add Interest
                </Button>
                <Button variant="outline" onClick={() => setShowAddInterest(false)}>
                  Cancel
                </Button>
              </div>
            </div>
            ...
            <div className="space-y-4">
              {interactionHistory.slice(0, 20).map((interaction) => (
                <Card key={interaction.id} className="p-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      <div className={`px-2 py-1 rounded-full ${getInteractionTypeColor(interaction.interaction_type)}`}>
              ...
              {interactionHistory.length === 0 && (
                <div className="text-center py-4">
                  ...
                  <p className="text-sm text-gray-500">No interaction history yet</p>
                  <p className="text-sm text-gray-400 mt-2">
