...
export const PipelineView: React.FC<PipelineViewProps> = ({ tenantId, userId }) => {
  ...
  useEffect(() => {
    fetchPipelineData();
  }, [tenantId, userId, searchTerm, selectedStatus, selectedPriority, sortBy, sortOrder]);
  ...
  const fetchPipelineData = async () => {
    try {
      ...
      if (!pipelineResponse.ok) throw new Error('Failed to fetch pipeline data');
  ...
  const handleBulkStageChange = async (applicationIds: string[], newStage: string) => {
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to bulk update stages');
    }
  };
