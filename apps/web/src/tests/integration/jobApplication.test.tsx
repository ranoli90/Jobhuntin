...
describe("Job Application Flow", () => {
  ...
  describe("Application Management", () => {
    ...
    it("should handle hold questions", async () => {
      const applicationsWithHold = mockApplications
        .map((app) => (app.status === "HOLD" ? app : null))
        .filter(Boolean);
    ...
    it("should submit hold answers", async () => {
      ...
      const answerTextarea = screen.getByPlaceholderText(
  ...
  describe("Error Handling", () => {
    it("should handle network errors gracefully", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText("Failed to record decision"),
        ).toBeInTheDocument();
      });
    });
  ...
  describe("Performance", () => {
    ...
    it("should handle large job lists efficiently", async () => {
      const largeJobList = Array.from({ length: 1000 }, (_, index) => ({
        id: `job-${index}`,
        title: `Job ${index}`,
        company: `Company ${index}`,
        salary_min: 80_000 + index * 1000,
        location: "Remote",
...
describe("Security", () => {
  ...
  it("should validate API responses", async () => {
    const maliciousResponse = {
      ...
      company: "EvilCorp",
      description:
        "Click here for malware: <a href=\"javascript:alert('xss')\">Link</a>",
    };
    ...
    await waitFor(() => {
      ...
      expect(
        screen.queryByText("Click here for malware:"),
      ).not.toBeInTheDocument();
    });
  });
});
