...
export function sanitizeSlug(
  ...
  slug = slug.replaceAll(/[^\d\sa-z-]/g, "");

  // Remove multiple spaces and hyphens
  slug = slug.replaceAll(/\s+/g, " ").replaceAll(/-+/g, "-");

  // Remove stop words if enabled
  if (options_.removeStopWords) {
    slug = slug
      .split(" ")
  ...
  if (options_.preventKeywordStuffing) {
  ...
  // Limit length
  if (options_.maxLength && slug.length > options_.maxLength) {
    slug = slug.slice(0, Math.max(0, options_.maxLength)).replace(/-+$/, "");
  }

  // Replace spaces with hyphens if allowed
  slug = options_.allowHyphens
    ? slug.replaceAll(/\s+/g, "-")
    : slug.replaceAll(/\s+/g, "");
...
export function detectSpammyPattern(url: string): boolean {
  ...
  if (pathParts.length >= 3 && pathParts[0] === "jobs") {
    ...
    for (const word of [...roleWords, ...locationWords]) {
    ...
    const hasUndefined =
      role.includes("undefined") || location.includes("undefined");

    return (
      hasRepeats || hasUndefined || !isValidSlug(role) || !isValidSlug(location)
    );
  }
 ...
 */
export function cleanUrlList(
  urls: string[],
): Array<{ original: string; cleaned: string; isSpammy: boolean }> {
  return urls.map((url) => ({
    original: url,
