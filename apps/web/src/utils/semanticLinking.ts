...
export function generateSemanticLinksForLocationRole(
  ...
  links.push({
    url: `/jobs/${role.toLowerCase().replaceAll(/\s+/g, "-")}`,
  ...
  links.push({
    url: `/jobs/all/${location.toLowerCase().replaceAll(/\s+/g, "-")}`,
  ...
  if (roleData?.category) {
    links.push({
      url: `/best/${roleData.category.toLowerCase().replaceAll(/\s+/g, "-")}`,
  ...
  for (const relatedRole of relatedRoles) {
    links.push({
      url: `/jobs/${relatedRole.toLowerCase().replaceAll(/\s+/g, "-")}/${location.toLowerCase().replaceAll(/\s+/g, "-")}`,
  ...
  for (const city of nearbyCities) {
    links.push({
      url: `/jobs/${role.toLowerCase().replaceAll(/\s+/g, "-")}/${city.toLowerCase().replaceAll(/\s+/g, "-")}`,
...
function getRelatedRoles(role: string): string[] {
  ...
  const normalizedRole = role.toLowerCase().replaceAll(/\s+/g, "-");
  return (
    roleClusters[normalizedRole] || [
      "senior-" + normalizedRole,
      "lead-" + normalizedRole,
...
function getNearbyCities(location: string): string[] {
  ...
  const normalizedLocation = location.toLowerCase().replaceAll(/\s+/g, "-");
...
export function calculateSemanticSimilarity(
  ...
  const commonWords = entity1
    .toLowerCase()
    .split(/\s+/)
    .filter((word) => entity2.toLowerCase().split(/\s+/).includes(word));

  const totalWords = new Set([
    ...entity1.toLowerCase().split(/\s+/),
    ...entity2.toLowerCase().split(/\s+/),
  ]).size;
 ...
 */
export function generateTopicalClusters(
  role: string,
  location: string,
): string[][] {
  return [
    ...
    [
      ...
      `best companies in ${location}`,
      `${location} startup jobs`,
