...
export function generateEnhancedContent(
  ...
  const { title, description } = generateSEOTitleAndDescription(role, city, roleData, cityData);
...
function generateUniqueValue(role: string, city: string, roleData?: RoleData, cityData?: CityData): string {
  ...
  if (cityData) {
    if (cityData.techHub) {
      insights.push(`${city} is a major tech hub with ${cityData.remotePercentage || 35}% remote opportunities`);
    }
    if (cityData.startupScene) {
      insights.push(`${city}'s startup ecosystem is thriving, with venture capital flowing into ${cityData.industries?.join(', ') || 'tech'}`);
    }
    if (cityData.costOfLiving && cityData.averageSalary) {
      const affordability = (cityData.averageSalary / cityData.costOfLiving) * 100;
      insights.push(`The salary-to-cost-of-living ratio in ${city} is ${affordability.toFixed(0)}%`);
    }
  }
  ...
  if (roleData) {
    if (roleData.growthRate) {
      insights.push(`${role} roles are projected to grow ${roleData.growthRate} through ${currentYear}`);
    }
    if (roleData.remotePercentage) {
      insights.push(`${roleData.remotePercentage}% of ${role} positions offer remote work flexibility`);
    }
    if (roleData.certifications && roleData.certifications.length > 0) {
      insights.push(`Top certifications for ${role}s include ${roleData.certifications.slice(0, 3).join(', ')}`);
    }
  }
  
  // Combined insight
  const combinedInsight = insights.length > 0 
    ? insights.join('. ')
...
function generateContentSections(role: string, city: string, roleData?: RoleData, cityData?: CityData): ContentSection[] {
  ...
  sections.push({
    ...
    wordCount: 180,
    keywords: [`${role} jobs ${city}`, `${city} employment`, `${role} career ${city}`],
  ...
  sections.push({
    ...
    wordCount: 175,
    keywords: [`${role} salary ${city}`, `${role} pay ${city}`, `${role} compensation`],
  ...
  sections.push({
    ...
    wordCount: 125,
    keywords: [`${role} companies ${city}`, `employers ${city} ${role}`, `hiring ${role}`],
  ...
  sections.push({
    ...
    wordCount: 125,
    keywords: [`${role} career ${city}`, `${role} advancement`, `${role} growth`],
...
function generateMarketOverview(role: string, city: string, roleData?: RoleData, cityData?: CityData): string {
  const currentYear = new Date().getFullYear();
  const demandLevel = roleData?.demandLevel || 'growing';
  const remotePercentage = roleData?.remotePercentage || 35;
  const industries = cityData?.industries || ['Technology', 'Healthcare', 'Finance'];
...
function generateSalaryContent(role: string, city: string, roleData?: RoleData, cityData?: CityData): string {
  const avgSalary = roleData?.avgSalary || 75000;
  const cityAvgSalary = cityData?.averageSalary || 80000;
  const isUS = cityData?.country === 'USA';
  const currency = isUS ? '$' : '€';
  
  const formatSalary = (val: number) => `${currency}${Math.round(val / 1000)}k`;
...
function generateEmployersContent(city: string, role: string, cityData?: CityData): string {
...
function generateCareerPathContent(role: string, city: string, roleData?: RoleData): string {
  ...
  return `The typical ${role} career path in ${city} progresses from ${careerPath[0]} to ${careerPath[careerPath.length - 1]} over 5-10 years. 
...
function generateFAQs(role: string, city: string, roleData?: RoleData, cityData?: CityData): FAQItem[] {
  return [
    {
      question: `What is the average ${role} salary in ${city}?`,
      answer: `The average ${role} salary in ${city} ranges from $${Math.round((roleData?.avgSalary || 75000) / 1000)}k to $${Math.round((roleData?.avgSalary || 75000) * 1.5 / 1000)}k, with experienced professionals earning at the higher end of the range.`
    },
    {
...
function generateLocalInsights(city: string, cityData?: CityData, role?: string, roleData?: RoleData): LocalInsight[] {
...
function generateSEOTitleAndDescription(role: string, city: string, roleData?: RoleData, cityData?: CityData): { title: string; description: string } {
  const currentYear = new Date().getFullYear();
  const demandLevel = roleData?.demandLevel || 'growing';
  ...
  Average salary: $${Math.round((roleData?.avgSalary || 75000) / 1000)}k. 
