  ...
  return (
    <>
      ...
      <ConfirmModal
        ...
        title="Trigger Job Sync"
        description={
          <>
            This will start a full job sync across all sources. It may take several
            minutes and use significant resources. Are you sure you want to continue?
          </>
        }
...
export default function AdminSourcesPage() {
  ...
  const triggerSync = useMutation({
    ...
    onError: (error_: Error) => {
            pushToast({
                title: "Sync Failed",
                description: error_.message || "Could not trigger sync.",
        tone: "error",
      });
      setIsSyncing(false);
    },
        ...
        <Card className="bg-red-50 border-red-200">
          <div className="flex items-center gap-3 text-red-800">
            ...
            <h3 className="font-bold">
              {isOffline ? "You appear to be offline" : "Error Loading Status"}
            </h3>
          </div>
          <p className="mt-2 text-red-600">
            {isOffline ? "Check your connection and try again." : String(error)}
          </p>
          <Button
            onClick={() => refetch()}
            disabled={isOffline}
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500">
            Monitor and manage job scraping integrations
          </p>
        </div>
        ...
        <Card className="p-6 border-slate-200 bg-white">
          <div className="flex justify-between items-start">
            <div>
              ...
              <h3 className="text-3xl font-bold text-slate-900 mt-1">
                {sources
                  .reduce((sum, s) => sum + s.total_jobs_fetched, 0)
                  .toLocaleString()}
              </h3>
            </div>
              ...
              <div className="flex items-center gap-2 mt-1">
                {sources.every((s) => s.consecutive_failures === 0) ? (
                  ...
                  <Badge variant="warning">
                    {sources.filter((s) => s.consecutive_failures > 0).length}{" "}
                    Degradations
                  </Badge>
                )}
              </div>
              ...
              {sources.map((source) => (
                ...
                >
                  <td className="px-6 py-4 font-medium text-slate-900 capitalize flex items-center gap-2">
                    <span
                      className={`w-2 h-2 rounded-full ${source.consecutive_failures === 0 ? "bg-emerald-500" : "bg-red-500"}`}
                    />
                    {source.source}
                  </td>
                  ...
                  <td className="px-6 py-4">
                    ...
                    ) : data?.circuit_breakers[source.source] ===
                      "half-open" ? (
      ...
      {/* Recent Activity Log */}
      <h3 className="font-bold text-slate-900 text-lg mt-8">
                    ...
                    className={`p-2 rounded-full ${
                      run.status === "completed"
                        ...
                        : run.status === "failed"
                          ...
                          : "bg-blue-100 text-blue-600 animate-spin-slow"
                    }`}
                  >
                    {run.status === "completed" ? (
