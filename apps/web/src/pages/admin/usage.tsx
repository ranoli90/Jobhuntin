        ...
        <Card className="p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-bold text-slate-900">
                {Math.round(
                  (usageData?.tenants.reduce(
                    (accumulator, t) => accumulator + t.quota_percentage,
                    0,
                  ) ?? 0) / (usageData?.tenants.length ?? 1),
                )}
                %
              </p>
            </div>
          ...
          {Array.from({ length: 30 }).map((_, index) => {
            ...
            return (
              <div
                ...
                style={{ height: `${height}%` }}
                title={`Day ${index + 1}: ${Math.round(height * 15)} matches`}
              />
            );
          })}
        </div>
