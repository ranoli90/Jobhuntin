...
export default function GuidePage() {
  ...
  const { data: guidesData, loading } = useDynamicData(
    () => import("../data/guides.json"),
  );
  const guides = guidesData as Record<
    string,
    {
      title: string;
      category: string;
  ...
  useEffect(() => {
    if (guide) {
      ...
      temporaryDiv.innerHTML = XSSProtection.sanitizeHTML(guide.content);
      const headingElements = temporaryDiv.querySelectorAll("h3, h4");
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 selection:bg-primary-500/20 selection:text-primary-700">
      ...
      <main className="max-w-4xl mx-auto px-6 py-16 sm:py-20">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-bold text-slate-900">
                  {author ? author.name : "JobHuntin Research Team"}
                </p>
        ...
        <div className="bg-white rounded-[2rem] sm:rounded-[2.5rem] p-6 sm:p-8 lg:p-10 border border-slate-100 shadow-sm mb-20">
          <h3 className="text-xl sm:text-2xl font-bold mb-6 font-display text-slate-900">
            Related Tools
          </h3>
          <div className="grid sm:grid-cols-2 gap-4">
            <Link
              to="/chrome-extension"
              className="flex items-center gap-4 p-4 rounded-2xl hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-100 group focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2"
            >
