...
export default function CareerPathPage() {
  ...
  const [selectedView, setSelectedView] = React.useState<
    "overview" | "trajectory" | "recommendations" | "learning"
  >("overview");
  ...
  const generateCareerTimeline = () => {
    ...
    timelineLine.className =
      "absolute left-8 top-1/2 bottom-0 w-0.5 h-px bg-slate-300";
    ...
    <div className="max-w-7xl mx-auto p-6 space-y-6">
      ...
      <div className="flex flex-col md:flex-row items-start justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-slate-500 font-medium">
            {t("careerPath.description", locale) ||
              "Visualize your career progression and get personalized recommendations for advancement"}
          </p>
        </div>
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="text-center">
                ...
                <p className="text-sm text-slate-600">
                  {t("careerPath.currentLevel", locale) || "Current Level"}
                </p>
                <div
                  className={`text-2xl font-bold ${getLevelColor(careerData.current_level)}`}
                >
                  {careerData.career_progression_score}
                </div>
                <p className="text-sm text-slate-600">
                  {t("careerPath.progressionScore", locale) ||
                    "Progression Score"}
                </p>
              </div>
              <div className="text-center">
                ...
                <p className="text-sm text-slate-600">
                  {t("careerPath.yearsExperience", locale) ||
                    "Years Experience"}
                </p>
              </div>
            </div>
              ...
              {careerData.trajectory.map((item, index) => (
                ...
                >
                  <div className="flex-shrink-0 w-16 text-center">
                    ...
                    <div
                      className={`w-3 h-3 rounded-full ${getLevelColor(item.level)} mx-auto`}
                    ></div>
                  </div>
                  <div className="flex-1">
                    <div>
                      ...
                      <span
                        className={`ml-2 text-sm ${getLevelColor(item.level)}`}
                      >
                        {item.company}
                      </span>
                    </div>
          ...
          <Card className="p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {careerData.possible_next_roles.map((role, index) => (
                <div
                  key={index}
                  className="p-4 border border-slate-200 rounded-lg hover:shadow-md"
                >
      ...
      {selectedView === "trajectory" && (
        <div className="space-y-6">
          <Card className="p-6">
            <h2 className="text-xl font-semibold text-slate-900 mb-4">
              {t("careerPath.careerProgression", locale) ||
                "Career Progression"}
            </h2>
                ...
                {careerData.trajectory.map((item, index) => (
                  <div key={index} className="relative">
                    ...
                    <div className="ml-4 pl-8 pb-4">
                      ...
                      <div className={`text-sm ${getLevelColor(item.level)}`}>
                        {item.level}
                      </div>
      ...
      {selectedView === "recommendations" && (
        <div className="space-y-6">
          <Card className="p-6">
            <h2 className="text-xl font-semibold text-slate-900 mb-4">
              {t("careerPath.recommendations", locale) ||
              ...
              <div className="flex items-center mb-2">
                ...
                <div>
                  ...
                  <span className="text-lg text-blue-900">
                    {careerData.recommendations.current_role}
                  </span>
                </div>
              </div>
              <div className="text-sm text-blue-700">
                <p className="font-medium">
                  {t("careerPath.pathType", locale) || "Path Type"}:
                </p>
                <span className="ml-2">
                  {careerData.recommendations.path_type}
                </span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <div>
                ...
                <p className="text-sm text-slate-600 mb-2">
                  {careerData.recommendations.estimated_timeline_months}{" "}
                  {t("careerPath.months", locale) || "months"}
                </p>
              </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <div className="text-sm text-slate-600">
                    {Math.round(careerData.recommendations.confidence * 100)}%
                  </div>
                  <div className="w-32 bg-slate-200 rounded-full h-2">
                    <div
                      className="bg-blue-600 h-2 rounded-full transition-all duration-300"
                      style={{
                        width: `${careerData.recommendations.confidence * 100}%`,
                      }}
                    ></div>
                  </div>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <div>
                <h4 className="font-semibold text-slate-900 mb-2">
                  {t("careerPath.salaryIncrease", locale) ||
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <div>
                      ...
                      <Badge
                        variant={
                          gap.importance === "high" ? "lagoon" : "outline"
                        }
                      >
                        {gap.importance}
                      </Badge>
                  ...
                  <p className="text-sm text-slate-600 mt-2">
                    {(gap as { resources?: string[] }).resources?.join(", ") ||
        ...
        <div className="space-y-6">
          <Card className="p-6">
            ...
            <div className="text-sm text-slate-600 mb-4">
              {t("careerPath.recommendedPace", locale) || "Recommended Pace"}:{" "}
              {careerData.learning_path.recommended_pace}
            </div>
            <div className="text-sm text-slate-600">
              {t("careerPath.totalWeeks", locale) || "Total Duration"}:{" "}
              {careerData.learning_path.total_weeks}
            </div>
          </Card>
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-sm text-slate-600 mt-2">
                      <span className="font-medium">
                        {t("careerPath.estimatedWeeks", locale) || "Estimated"}:
                      </span>{" "}
                      {milestone.estimated_weeks}{" "}
                      {t("careerPath.weeks", locale) || "weeks"}
                    </div>
                    <div className="text-sm text-slate-600 mt-2">
                      <span className="font-medium">
                        {t("careerPath.resources", locale) || "Resources"}:
                      </span>{" "}
