...
function getConfidenceLevel(
  ...
  if (confidence >= 0.8) {
    return {
      label: t("onboarding.highConfidence", locale) || "HIGH",
      color: "text-emerald-600",
      bgColor: "bg-emerald-100",
    };
  } else if (confidence >= 0.5) {
    return {
      label: t("onboarding.mediumConfidence", locale) || "MEDIUM",
      color: "text-amber-600",
      bgColor: "bg-amber-100",
    };
  } else {
    return {
      label: t("onboarding.lowConfidence", locale) || "LOW",
...
function SkillRow({ skill, onEdit, onDelete, locale }: SkillRowProperties) {
  return (
    <motion.div
      layout
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex flex-wrap gap-x-2 md:gap-x-3 gap-y-0.5 text-[10px] md:text-xs text-slate-500 mt-0.5 md:mt-1">
          ...
          {skill.project_count > 0 && (
            <span>
              {skill.project_count}{" "}
              {t("onboarding.projects", locale) || "projects"}
            </span>
          )}
        </div>
      </div>
...
interface AddSkillFormProperties {
  onAdd: (skill: RichSkill) => void;
  onCancel: () => void;
  locale: string;
}

function AddSkillForm({ onAdd, onCancel, locale }: AddSkillFormProperties) {
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <span className="font-bold text-emerald-800 text-sm">
          {t("onboarding.addSkill", locale) || "Add a Skill"}
        </span>
      </div>
      <div className="grid gap-2 md:grid-cols-3">
        <div>
          <label htmlFor="add-skill-name" className="sr-only">
            {t("onboarding.skillNamePlaceholder", locale) || "Skill name"}
          </label>
          <Input
            id="add-skill-name"
            type="text"
        ...
        <div>
          <label htmlFor="add-skill-years" className="sr-only">
            {t("onboarding.yearsPlaceholder", locale) || "Years"}
          </label>
          <Input
            id="add-skill-years"
            type="number"
            ...
            inputMode="numeric"
            aria-label={
              t("onboarding.yearsExperience", locale) || "Years of experience"
            }
            aria-valuetext={
              years
                ? `${years} ${t("onboarding.years", locale) || "years"}`
                : undefined
            }
          />
        </div>
        <div>
          <label htmlFor="add-skill-context" className="sr-only">
            {t("onboarding.contextPlaceholder", locale) || "Context (optional)"}
          </label>
          <Input
      ...
      <div className="flex flex-col sm:flex-row justify-end gap-2">
        <Button
          ...
          variant="ghost"
          onClick={onCancel}
          className="min-h-[44px] h-11 sm:h-12 text-xs touch-manipulation"
        >
        ...
        <Button
          ...
          className="min-h-[44px] h-11 sm:h-12 text-xs bg-emerald-600 hover:bg-emerald-500 touch-manipulation"
        >
...
export interface SkillReviewStepProperties {
  onNext: () => void;
  onPrev: () => void;
...
export function SkillReviewStep({
  ...
  setRichSkills,
  isSaving,
}: SkillReviewStepProperties) {
  const locale = getLocale();
  const [isAddingSkill, setIsAddingSkill] = React.useState(false);
  const [editingIndex, setEditingIndex] = React.useState<number | null>(null);

  const config = (s: { confidence?: number }) => s.confidence ?? 0;
  const highSkills = richSkills.filter((s) => config(s) >= 0.8);
  const mediumSkills = richSkills.filter(
    (s) => config(s) >= 0.5 && config(s) < 0.8,
  );
  const lowSkills = richSkills.filter((s) => config(s) < 0.5);
  ...
  const handleAddSkill = (skill: RichSkill) => {
    if (richSkills.length >= MAX_SKILLS) {
      pushToast({
        ...
        description:
          t("onboarding.maxSkillsDesc", locale) ||
          `You can add up to ${MAX_SKILLS} skills. Remove some to add more.`,
        tone: "warning",
      });
      return;
    }
    ...
    if (isDuplicate) {
      ...
      pushToast({
        title:
          t("onboarding.skillExistsTitle", locale) || "Skill already exists",
    ...
    setRichSkills((previous) => [...previous, skill]);
    setIsAddingSkill(false);
  };

  const handleDeleteSkill = (skill: RichSkill) => {
    setRichSkills((previous) => previous.filter((s) => s !== skill));
  };

  const handleUpdateSkill = (index: number, updates: Partial<RichSkill>) => {
    setRichSkills((previous) =>
      previous.map((s, index_) =>
        index_ === index ? { ...s, ...updates } : s,
      ),
    );
    ...
    <div role="region" aria-labelledby="skill-review-step-title">
      <div className="mb-4 md:mb-6 flex items-center gap-3 md:gap-4 border-b border-slate-100 pb-4 md:pb-6">
        ...
        <div className="min-w-0">
          ...
          <p className="text-xs md:text-sm text-slate-500 font-medium">
            {formatT(
              "onboarding.skillsCount",
              { count: richSkills.length },
              locale,
            )}
          </p>
        </div>
      ...
      {richSkills.length === 0 ? (
        <div className="text-center py-8 md:py-12">
          ...
          <p className="text-slate-600 text-sm font-medium mb-1">
            {t("onboarding.noSkillsTitle", locale) || "No skills detected"}
          </p>
          <p className="text-slate-400 text-xs mb-4">
            {t("onboarding.noSkillsDesc", locale) ||
              "Add your skills manually to improve job matching"}
          </p>
          <Button
            ...
            <div className="p-2.5 md:p-3 rounded-xl md:rounded-2xl bg-emerald-50/50 border border-emerald-100">
              <div className="flex items-center gap-2 mb-2 px-1">
                ...
                <span className="text-[10px] md:text-xs font-bold text-emerald-700 uppercase tracking-wide">
                  {t("onboarding.highConfidenceLabel", locale) ||
                    "High Confidence"}
                </span>
              ...
              <div className="space-y-1.5 md:space-y-2">
                {highSkills.map((skill) => {
                  const globalIndex = richSkills.indexOf(skill);
                  return (
                    <SkillRow
                      key={`${skill.skill}-${globalIndex}`}
                      skill={skill}
            ...
            <div className="p-2.5 md:p-3 rounded-xl md:rounded-2xl bg-amber-50/50 border border-amber-100">
              <div className="flex items-center gap-2 mb-2 px-1">
                ...
                <span className="text-[10px] md:text-xs font-bold text-amber-700 uppercase tracking-wide">
                  {t("onboarding.mediumConfidenceLabel", locale) ||
                    "Medium Confidence"}
                </span>
            ...
            <div className="p-2.5 md:p-3 rounded-xl md:rounded-2xl bg-red-50/50 border border-red-100">
              <div className="flex items-center gap-2 mb-2 px-1">
                ...
                <span className="text-[10px] md:text-xs font-bold text-red-700 uppercase tracking-wide">
                  {t("onboarding.lowConfidenceLabel", locale) ||
                    "Low Confidence"}
                </span>
                <span className="text-[10px] md:text-xs text-red-400">
                  ({lowSkills.length}) —{" "}
                  {t("onboarding.reviewRecommended", locale) ||
                    "review recommended"}
                </span>
              </div>
              <div className="space-y-1.5 md:space-y-2">
      ...
      {!isAddingSkill && (
        <Button
          ...
          aria-label={
            t("onboarding.addMissingSkill", locale) || "Add a new skill"
          }
        >
          ...
          <div className="flex items-start gap-2">
            ...
            <div>
              ...
              <p className="text-[10px] md:text-xs text-amber-600 mt-0.5 leading-relaxed">
                {t("onboarding.noSkillsHelp", locale) ||
      ...
      <div className="flex flex-col sm:flex-row gap-3 pt-4 mt-4">
        ...
        <Button
          ...
          aria-label={
            richSkills.length === 0
              ? t("onboarding.skipSkills", locale)
              : t("onboarding.saveSkills", locale)
          }
