...
const getBehavioralQuestions = (
  ...
  },
  {
    ...
    options: [
      {
        value: "early_startup",
        label: t("onboarding.workStyleQ3Option1", locale),
      },
      { value: "growth", label: t("onboarding.workStyleQ3Option2", locale) },
      {
        value: "enterprise",
        label: t("onboarding.workStyleQ3Option3", locale),
      },
  ...
  },
  {
    ...
    options: [
      ...
      {
        value: "methodical",
        label: t("onboarding.workStyleQ5Option3", locale),
      },
...
function computeArchetype(answers: Record<string, string>): Archetype {
  ...
  if (autonomy === "high" && ownership === "solo" && pace === "fast") {
    return {
      ...
      icon: Rocket,
      description: "You thrive when given ownership and freedom to ship fast.",
  ...
  if (ownership === "team" && answers.communication_style === "sync") {
    return {
      ...
      icon: Users,
      description: "You shine when working closely with a team in real-time.",
  ...
  if (
    ownership === "lead" ||
    trajectory === "manager" ||
    trajectory === "tech_lead"
  ) {
    return {
      name: "The Strategic Guide",
      icon: Compass,
      description: "You lead by example and guide others to deliver.",
      gradient: "from-amber-600 to-orange-600",
    };
  }
  if (pace === "methodical" && answers.learning_style === "docs") {
    return {
      name: "The Thoughtful Craftsman",
      icon: Microscope,
      description: "You value quality and thoroughness above speed.",
      gradient: "from-emerald-600 to-teal-600",
    };
  }
  if (stage === "early_startup" || trajectory === "founder") {
    return {
      name: "The Startup Maverick",
      icon: Zap,
      description: "You love the chaos and ownership of building from zero.",
  ...
  if (stage === "enterprise" && pace === "steady") {
    return {
      ...
      icon: Building2,
      description: "You excel at scaling and structuring large systems.",
      gradient: "from-slate-600 to-slate-800",
    };
  }
  return {
    name: "The Adaptive Pro",
    icon: Sparkles,
    description: "You're versatile and can thrive in any environment.",
    gradient: "from-primary-600 to-purple-600",
  };
}

interface WorkStyleStepProperties {
  onNext: () => void;
  onPrev: () => void;
...
export function WorkStyleStep({
  ...
  const allQuestions = [
    ...
    { ...TRAJECTORY_QUESTION, maps_to: "career_trajectory" },
  ];
  ...
  const handleAnswer = (
    ...
    setAnswers((previous) => ({ ...previous, [mapsTo]: optionValue }));
    ...
    if (currentQuestion < totalQuestions - 1) {
      ...
      setTimeout(() => {
        setCurrentQuestion((current) => {
          for (let index = current + 1; index < totalQuestions; index++) {
            if (
              !answersSnapshot[allQuestions[index].maps_to] &&
              allQuestions[index].maps_to !== mapsTo
            ) {
              return index;
            }
          }
          return Math.min(current + 1, totalQuestions - 1);
        });
      }, 600);
    }
  };
  ...
  return (
    <div
      ...
      role="region"
      aria-labelledby="work-style-step-title"
    >
      <div className="flex-1">
        <div className="mb-3 md:mb-6 flex items-center gap-2.5 md:gap-5 border-b border-slate-100 pb-2.5 md:pb-6">
          ...
          <div className="min-w-0">
            <h2
              id="work-style-step-title"
              className="font-display text-lg md:text-3xl font-black text-slate-900 tracking-tight truncate"
            >
        ...
        <div className="flex gap-1.5 mb-4 md:mb-6 justify-center">
          {allQuestions.map((q, index) => (
            <button
              ...
              aria-label={`${t("onboarding.question", locale) || "Question"} ${index + 1}`}
          ...
          >
            ...
            >
              {"options" in question &&
                ...
                (question.options as { value: string; label: string }[]).map(
                  (option) => {
                    const isSelected =
                      answers[question.maps_to] === option.value;
                    return (
                      <button
                        key={option.value}
                        type="button"
                        ...
                        className={`w-full min-h-[44px] p-3 md:p-4 rounded-xl text-left transition-all border-2 ${
                          isSelected
        ...
        <p className="mt-4 md:mt-6 text-center text-[10px] md:text-xs text-slate-400">
          {t("onboarding.questionCounter", locale)
          ...
          {isComplete &&
            (() => {
              ...
              return (
                ...
                >
                  <div
                    className={`absolute inset-0 bg-gradient-to-br ${archetype.gradient}`}
                  />
                  <div className="relative z-10">
                    ...
                    <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-white/50 mb-1">
                      {t("onboarding.yourWorkStyle", locale) ||
                        "Your Work Style"}
                    </p>
      ...
      <div className="flex flex-col sm:flex-row gap-3 md:gap-4 pt-2 md:pt-4 shrink-0 mt-auto sticky bottom-0 bg-gradient-to-t from-white via-white/95 to-transparent backdrop-blur">
        ...
        <Button
          ...
          aria-label={t("onboarding.saveWorkStyle", locale)}
