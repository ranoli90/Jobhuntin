...
export default function InterviewSimulatorPage() {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      return await apiGet<InterviewSessionDetail>(
        `interviews/sessions/${selectedSession}`,
      );
    },
  ...
  const submitAnswerMutation = useMutation({
    mutationFn: async ({
      ...
      return await apiPost<{ is_complete?: boolean }>(
      ...
      <div className="max-w-4xl mx-auto p-6">
        <Card className="p-6 text-center">
          ...
          <h2 className="text-xl font-semibold text-red-600 mb-2">
            {t("interviewSimulator.errorLoading", locale) ||
      ...
      <div className="flex flex-col md:flex-row items-start justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-slate-500 font-medium">
            {t("interviewSimulator.description", locale) ||
          ...
          {sessions.length === 0 ? (
            <Card className="p-8 text-center">
              ...
              <h3 className="text-xl font-semibold text-slate-900 mb-2">
                {t("interviewSimulator.noSessions", locale) ||
                  "No Sessions Yet"}
              </h3>
              <p className="text-slate-600 mb-4">
                {t("interviewSimulator.noSessionsDescription", locale) ||
                  "Start your first interview practice session to get personalized feedback"}
              </p>
              <Button onClick={handleCreateSession}>
                <Play className="w-4 h-4 mr-2" />
                {t("interviewSimulator.startFirstSession", locale) ||
                  "Start First Session"}
              </Button>
            </Card>
          ) : (
                ...
                <div className="flex justify-between items-start mb-3">
                  <div>
                    ...
                    <div className="flex items-center gap-2 text-sm text-slate-500">
                      <Badge
                        variant={
                          session.status === "completed" ? "lagoon" : "outline"
                        }
                      >
                        {session.status}
                      </Badge>
                  ...
                  <div className="mt-3 pt-3 border-t border-slate-200">
                    <div className="flex items-center justify-between text-sm">
                      <span className="font-medium">
                        {t("interviewSimulator.overallScore", locale) ||
                          "Overall Score"}
                        :
                      </span>
                      <span
                        className={`font-bold ${getScoreColor(session.total_score ?? 0)}`}
                      >
                        {Math.round(session.total_score ?? 0)}
                      </span>
                    </div>
                    <div className="text-xs text-slate-500">
                      {t("interviewSimulator.duration", locale) || "Duration"}:{" "}
                      {formatTime((session.duration_minutes ?? 0) * 60)}
                    </div>
                  </div>
                )}
              </Card>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-semibold text-slate-900">
                {t("interviewSimulator.sessionDetail", locale) ||
                  "Session Practice"}
              </h2>
              <div className="flex items-center gap-2">
                <Button
            ...
            {detailLoading ? (
              ...
              <Card className="p-6">
                ...
                <div className="mb-6">
                  ...
                  <div className="w-full bg-slate-200 rounded-full h-2">
                    <div
                      ...
                      style={{
                        width: `${((currentQuestionIndex + 1) / (sessionDetail.questions?.length || 1)) * 100}%`,
                      }}
                    ></div>
                  </div>
                </div>
                ...
                {sessionDetail.questions &&
                  currentQuestionIndex < sessionDetail.questions.length && (
                    <div className="mb-6">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-lg font-semibold text-slate-900">
                          {t("interviewSimulator.question", locale) ||
                            "Question"}{" "}
                          {currentQuestionIndex + 1}
                        </span>
                        <Badge variant="outline" className="ml-2">
                          {sessionDetail.questions[currentQuestionIndex]
                            ?.difficulty || "medium"}
                        </Badge>
                      </div>

                      {sessionDetail.responses?.[currentQuestionIndex] && (
                        <div className="text-sm text-slate-500">
                          <span className="font-medium">
                            {t("interviewSimulator.previousAnswer", locale) ||
                              "Previous answer"}
                            :
                          </span>
                        ...
                        {!sessionDetail.questions[currentQuestionIndex]
                          ...
                          <div className="mb-4">
                            <textarea
                              ...
                              placeholder={
                                t(
                                  "interviewSimulator.typeYourAnswer",
                                  locale,
                  ...
                  <Card className="mt-4 p-4 bg-blue-50 border-blue-200">
                    <div className="flex items-center mb-2">
                      ...
                      <h4 className="text-lg font-semibold text-blue-900">
                        {t("interviewSimulator.aiFeedback", locale) ||
                          "AI Feedback"}
                      </h4>
                    </div>
                    ...
                    <div className="text-sm text-slate-700">
                      <p className="mb-2">
                        <span className="font-medium">
                          {t("interviewSimulator.feedback", locale) ||
                            "Feedback"}
                          :
                        </span>
                        {sessionDetail.responses[currentQuestionIndex]?.feedback
                          ?.feedback ||
                          t("interviewSimulator.goodAnswer", locale) ||
                          "Good answer! Consider the clarity and relevance."}
                      </p>
                      ...
                      {sessionDetail.responses[currentQuestionIndex]?.feedback
                        ...
                        <div>
                          <span className="font-medium">
                            {t("interviewSimulator.suggestions", locale) ||
                              "Suggestions"}
                            :
                          </span>
                          <ul className="list-disc list-inside ml-4 space-y-1">
                            {sessionDetail.responses[
                              currentQuestionIndex
                      ...
                      {sessionDetail.responses[currentQuestionIndex]
                        ...
                        <div className="mt-3 pt-3 border-t border-blue-300">
                          <span className="font-medium">
                            {t("interviewSimulator.nextQuestion", locale) ||
                              "Next Question"}
                            :
                          </span>
                  ...
                  <Card className="mt-6 p-6 bg-green-50 border-green-200">
                    <div className="flex items-center mb-4">
                      ...
                      <h4 className="text-xl font-semibold text-green-900">
                        {t("interviewSimulator.sessionCompleted", locale) ||
                          "Session Completed"}
                      </h4>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        <h5 className="font-semibold text-slate-900 mb-2">
                          {t("interviewSimulator.performance", locale) ||
                            "Performance"}
                        </h5>
                        <div className="space-y-2">
                          <div className="flex justify-between">
                            ...
                            <span
                              className={`font-bold text-2xl ${getScoreColor(sessionDetail.total_score || 0)}`}
                            >
                          ...
                          <div className="flex justify-between">
                            <span className="font-medium">
                              {t(
                                "interviewSimulator.questionsAnswered",
                                locale,
                              ) || "Questions Answered"}
                              :
                            </span>
                            <span className="font-bold">
                              {sessionDetail.questions_answered || 0}
                            </span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div>
                        <h5 className="font-semibold text-slate-900 mb-2">
                          {t("interviewSimulator.strengths", locale) ||
                            "Strengths"}
                        </h5>
                        <div className="space-y-1">
                          {sessionDetail.top_strengths?.map(
                      ...
                      <div>
                        <h5 className="font-semibold text-slate-900 mb-2">
                          {t("interviewSimulator.improvements", locale) ||
                            "Areas for Improvement"}
                        </h5>
                        <div className="space-y-1">
                          {sessionDetail.top_improvements?.map(
