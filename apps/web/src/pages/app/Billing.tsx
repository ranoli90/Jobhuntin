...
import { ArrowUpRight, CheckCircle, AlertTriangle, FileText, Download, Loader2 } from "lucide-react";
          ...
          <div className="grid lg:grid-cols-3 gap-6 lg:gap-8">
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="grid md:grid-cols-3 gap-4">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="p-6 border border-slate-200 dark:border-slate-800 rounded-2xl animate-pulse">
  ...
  return (
    <Page title="Billing & Quota">
      <div className="max-w-6xl mx-auto space-y-6 pb-6 px-4 lg:px-0">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div>
            <h2 className="text-2xl font-black text-slate-900 dark:text-slate-100 tracking-tight">Billing & Quota</h2>
            <p className="text-slate-500 dark:text-slate-400 font-medium">Manage your subscription, usage, and invoices.</p>
          </div>
        ...
        {error && (
          <div className="flex items-center gap-3 p-4 rounded-2xl bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-900 text-red-800 dark:text-red-200" role="alert">
        ...
        <div className="grid lg:grid-cols-3 gap-6 lg:gap-8">
          <div className="lg:col-span-2 space-y-6 lg:space-y-8">
            <Card className="p-6 lg:p-8 border-slate-200 dark:border-slate-800" shadow="sm">
              <h3 className="text-xl font-black text-slate-900 dark:text-slate-100 mb-6 font-display">Current Allocation</h3>
              <div className="space-y-4">
                <div className="flex justify-between items-end">
                  <p className="text-sm font-bold text-slate-500 dark:text-slate-400 uppercase">Monthly Volume</p>
                  <p className="text-sm font-black text-slate-900 dark:text-slate-100">{usageUsed} / {usageLimit}</p>
                </div>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6">
              {tiers.map((tier) => (
                ...
                >
                  <h4 className="font-black text-slate-900 dark:text-slate-100 text-lg mb-1">{tier.name}</h4>
                  <p className="text-3xl font-black text-slate-900 dark:text-slate-100 mb-6">{tier.price}</p>
                  <ul className="space-y-3 mb-8 flex-1">
                    {tier.features.map((f) => (
                      <li key={f} className="text-xs text-slate-500 dark:text-slate-400 font-medium flex items-center gap-2">
            ...
            {/* Invoices section */}
            <Card className="p-6 lg:p-8 border-slate-200 dark:border-slate-800" shadow="sm">
              <h3 className="text-xl font-black text-slate-900 dark:text-slate-100 mb-4 font-display">Invoice History</h3>
                      ...
                      <TableRow>
                        <TableCell colSpan={3} className="text-center py-12">
                          <div className="flex flex-col items-center gap-3">
                            ...
                            <div>
                              <p className="text-slate-900 dark:text-slate-100 font-medium">No invoices yet</p>
                              <p className="text-sm text-slate-500 dark:text-slate-400">Your billing history will appear here</p>
                            </div>
                          </div>
                        </TableCell>
                      </TableRow>
