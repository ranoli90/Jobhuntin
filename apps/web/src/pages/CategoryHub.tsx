...
function generateFAQ(
  ...
  competitorsMap: Record<string, CompetitorItem>,
): FAQItem[] {
  ...
  return [
    ...
    },
    {
      ...
      answer: `Several ${category.name.toLowerCase()} offer free tiers: ${
        category.competitors
          .filter((slug) => competitorsMap[slug]?.pricing?.free_tier)
          .map((slug) => competitorsMap[slug]?.name)
          .filter(Boolean)
          .join(", ") || "limited options available"
      }. JobHuntin also offers a free tier that includes access to the AI agent. For unlimited applications with stealth mode, Pro plans start at $19/month.`,
    },
    {
...
export default function CategoryHub() {
  ...
  const { data: categoriesData, loading: loadingCategories } = useDynamicData(
  ...
  const CATEGORIES_MAP: Record<string, CategoryItem> = Object.fromEntries(
    categories.map((c) => [c.slug, c]),
  );
  const COMPETITORS_MAP: Record<string, CompetitorItem> = Object.fromEntries(
    competitorsList.map((c) => [c.slug, c]),
  );
      ...
      <main className="max-w-5xl mx-auto px-6 py-24">
        ...
        >
          ...
          <p className="text-xl text-slate-500 max-w-2xl mx-auto font-medium">
            {category.description}. We tested {competitors.length}+ tools and
            ranked them by automation level, quality, stealth capability, and
            value.
          </p>
        </motion.div>
        ...
        >
          ...
          <div className="bg-white rounded-3xl border border-slate-100 p-8 shadow-sm">
            ...
            <div className="flex flex-wrap gap-2">
              {(seoData.contentSections?.[0]?.keywords ?? [])
                .slice(0, 5)
                .map((keyword: string) => (
                  <span
                    key={keyword}
            ...
            <div className="flex flex-col md:flex-row gap-8 items-start">
              <div className="flex-1">
                ...
                <p className="text-slate-500 font-medium mb-4">
                  Autonomous AI job search automation with stealth mode,
                  per-application resume tailoring, and 24/7 background
                  operation. The most comprehensive tool in this category.
                </p>
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  {[
                    ...
                    <span
                      key={tag}
                      className="bg-white text-primary-600 text-xs font-bold px-3 py-1 rounded-full border border-primary-100"
                    >
                      {tag}
                    </span>
                  ))}
        ...
        >
          <h2 className="text-3xl font-black text-slate-900 mb-6">
            {seoData.h2s?.[1] ?? "What Makes a Great Category"}
          </h2>
          <div className="bg-white rounded-3xl border border-slate-100 p-8 shadow-sm">
            <p className="text-lg text-slate-600 leading-relaxed mb-6">
            ...
            <div className="flex flex-wrap gap-2">
              {(seoData.contentSections?.[1]?.keywords ?? [])
                .slice(0, 5)
                .map((keyword: string) => (
                  <span
                    key={keyword}
        ...
        <div className="mb-16">
          <h2 className="text-3xl font-black text-slate-900 mb-8">
            {seoData.h2s?.[2] ?? "Top Tools Compared"}
          </h2>
              ...
              const score = rating
                ? Math.round(
                    (Object.values(rating).reduce(
                      (sum: number, vals: number[]) => sum + (vals[0] ?? 0),
                      0,
                    ) /
                  ...
                  <div className="flex flex-col md:flex-row gap-6 items-start">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex flex-wrap gap-4 text-sm text-slate-600">
                        ...
                        <span className="flex items-center gap-1">
                          {comp.features?.resume_tailoring ? (
                            <Check className="w-4 h-4 text-emerald-500" />
                          ) : (
                            <X className="w-4 h-4 text-red-400" />
                          )}
        ...
        >
          <h2 className="text-3xl font-black text-slate-900 mb-6">
            {seoData.h2s?.[3] ?? "Pricing Comparison"}
          </h2>
          <div className="bg-white rounded-3xl border border-slate-100 p-8 shadow-sm">
            <p className="text-lg text-slate-600 leading-relaxed mb-6">
            ...
            <div className="flex flex-wrap gap-2">
              {(seoData.contentSections?.[2]?.keywords ?? [])
                .slice(0, 5)
                .map((keyword) => (
                  <span
                    key={keyword}
