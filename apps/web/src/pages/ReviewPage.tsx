...
export default function ReviewPage() {
  ...
  const { data: competitorsData, loading } = useDynamicData(
        ...
        schema={[
          ...
          },
          {
            ...
            offers: {
              "@type": "Offer",
              price:
                competitor.pricing?.starts_at === "Free" ||
                !competitor.pricing?.starts_at
                  ? "0"
                  : competitor.pricing.starts_at.replace(/[^0-9.]/g, ""),
              priceCurrency: "USD",
              availability:
      ...
      <main className="max-w-5xl mx-auto px-6 py-24">
        ...
        >
          ...
          <p className="text-lg text-slate-500 mb-8 font-medium leading-relaxed max-w-3xl">
            {competitor.tagline}. We tested {competitor.name} extensively and
            rated it against JobHuntin across speed, quality, automation level,
            stealth capability, and value for money. Here's our honest
            assessment.
          </p>
                ...
                ([key, ratings]) => {
                  ...
                  return (
                    <div key={key} className="flex items-center gap-3">
                      <span className="text-sm text-slate-500 capitalize w-24">
                        {key.replace("_", " ")}
                      </span>
          ...
          {competitor.status === "discontinued" && (
            <div className="mt-6 bg-red-50 border border-red-200 rounded-2xl p-4 text-red-700 text-sm font-medium">
              ⚠️ <strong>Note:</strong> {competitor.name} has been discontinued
              and is no longer accepting new users.
              <Link
                to={`/alternative-to/${competitorSlug}`}
                className="text-red-800 underline ml-1 font-bold"
              >
        ...
        >
          ...
          <div className="bg-emerald-50 rounded-3xl border border-emerald-100 p-8">
            <ul className="space-y-4">
              {(competitor.strengths ?? []).map((s, index) => (
                                <li key={index} className="flex items-start gap-3">
                  <span className="text-emerald-500 font-bold text-lg leading-none mt-0.5">
                    +
                  </span>
        ...
        >
          ...
          <div className="bg-white rounded-3xl border border-slate-100 p-8">
            ...
            <ul className="space-y-2">
              {(competitor.pricing?.tiers ?? []).map(
                (t: { name?: string; price?: string }, index: number) => (
                  <li key={index} className="text-slate-600 text-sm font-medium flex items-center gap-2">
            ...
            <div className="mt-6 pt-6 border-t border-slate-100">
              <p className="text-sm text-slate-500 font-medium">
                Compare pricing →{" "}
                <Link
                  to={`/pricing-vs/${competitorSlug}`}
                  className="text-primary-600 font-bold hover:underline"
                >
                  {competitor.name} vs JobHuntin Pricing
                </Link>
              </p>
            </div>
          </div>
        ...
        >
          ...
          <ComparisonTable
            competitor={{ ...competitor, features: competitor.features ?? {} }}
            variant="compact"
          />
          <p className="text-center mt-4">
            <Link
              to={`/vs/${competitorSlug}`}
              className="text-primary-600 font-bold text-sm hover:underline inline-flex items-center gap-1"
            >
        ...
        <InternalLinkMesh
          ...
          currentType="review"
          competitorCategory={
            competitor.category ? [competitor.category] : undefined
          }
        />
