...
export default function Login() {
  ...
  const [successState, setSuccessState] = useState<{ email: string } | null>(
    null,
  );
  ...
  useEffect(() => {
    if (sessionStorage.getItem("session_expired") === "true") {
      ...
      pushToast({
        title: t("login.sessionExpired", getLocale()),
        description: t("login.signInAgain", getLocale()),
        tone: "info",
      });
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const hint = searchParameters.get("hint");
    if (error === "auth_failed") {
      ...
      const messages: Record<string, string> = {
        ...
        ip_mismatch:
          "This link was opened from a different device or network. Request a new one from the same place you signed in.",
      };
      setFormError(
        messages[hint || ""] ||
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      setRateLimitCountdown((previous) => {
    ...
    if (!result.success) {
      ...
      if (result.captchaRequired) {
        ...
        const error = new Error(
          result.error || "Please complete the captcha verification",
        ) as Error & { status?: number };
        error.status = result.status;
        throw error;
      }
      const error = new Error(result.error || "Magic link failed") as Error & {
        status?: number;
      };
      error.status = result.status;
      throw error;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (typoSuggestion) {
      ...
      pushToast({
        title: "Email corrected",
        description: `Sent to ${emailToSend}`,
        tone: "info",
      });
    }
    ...
    try {
      ...
      let message =
        "We couldn't send your magic link. Please check your email and try again.";
      const m = error_.message || "";
      ...
      if (
        ...
        message = "Please complete the captcha verification and try again.";
  ...
  if (successState) {
    return (
      <>
        <SEO
          title="Check Your Email | JobHuntin"
          description="We sent you a magic link. Check your inbox to sign in."
          noindex
        />
        <div
        ...
        >
          ...
          <div
            ...
            style={{
              background:
                "radial-gradient(ellipse 80% 50% at 50% 0%, rgba(69,93,211,0.12) 0%, transparent 60%)",
            }}
          />
          <motion.div
          ...
          >
            <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm p-8 sm:p-10">
              ...
              <div className="space-y-3">
                ...
                >
                  {resendLoading
                    ? t("login.sending", getLocale())
                    : (rateLimitCountdown
                      ? formatT(
                          "login.resendIn",
                          { seconds: String(rateLimitCountdown) },
                          getLocale(),
                        )
                      : t("login.resendLink", getLocale()))}
                </Button>
                <Button
                  variant="outline"
                  onClick={() => setSuccessState(null)}
                  className="w-full border-white/30 text-white hover:bg-white/10 focus:ring-2 focus:ring-[#455DD3] focus:outline-none"
                >
    ...
    return (
      <>
        ...
        <div
          ...
          role="status"
          aria-live="polite"
          aria-label="Verifying your sign-in link"
        >
          <div className="text-center space-y-4 max-w-sm px-6">
            <Loader2
              className="w-14 h-14 text-[#7DD3CF] animate-spin mx-auto"
              aria-hidden
            />
            ...
            <p className="text-white/70">
              Please wait while we sign you in. This usually takes a few
              seconds.
            </p>
  ...
  return (
    <>
      <SEO
        title="Sign In | JobHuntin"
        description="Sign in to JobHuntin to access your job search dashboard."
        noindex
      />
      <a
        href="#login-form"
        className="sr-only focus:not-sr-only focus:absolute focus:top-4 focus:left-4 focus:z-50 focus:px-4 focus:py-2 focus:bg-[#455DD3] focus:text-white focus:rounded-lg focus:font-medium"
      >
        Skip to login form
      </a>
      ...
      <div className="min-h-screen flex">
        ...
        <div
          className="hidden lg:flex lg:w-[48%] xl:w-[45%] relative overflow-hidden"
          style={{
            background:
              "linear-gradient(165deg, #0F1729 0%, #1A2744 50%, #0d1320 100%)",
          }}
        ...
        <div className="w-full lg:w-[52%] xl:w-[55%] flex flex-col p-6 sm:p-8 lg:p-12 xl:p-16 bg-[#F7F6F3] dark:bg-slate-900 relative">
          ...
          <div
            className="lg:hidden absolute top-0 left-0 right-0 h-32 pointer-events-none bg-gradient-to-b from-[#455DD3]/5 to-transparent dark:from-[#455DD3]/10"
            aria-hidden
          />
          ...
          <div className="w-full max-w-[400px] mx-auto flex-1 flex flex-col justify-center pt-16 lg:pt-0">
            <div className="lg:hidden text-center mb-8">
              ...
              />
              <h1
                className="text-2xl font-bold text-[#2D2A26] dark:text-slate-100 mb-2 tracking-tight"
            ...
            >
              ...
              >
                <div className="relative">
                  <label
                    htmlFor="login-email"
                    className="block text-sm font-semibold text-[#2D2A26] dark:text-slate-100 mb-2"
                  >
                  ...
                  >
                    <input
                      ...
                      onPaste={(e) => {
                        ...
                        if (
                          pasted &&
                          ValidationUtils.validate.email(pasted).isValid
                        ) {
                          e.preventDefault();
                      ...
                      aria-activedescendant={
                        showSuggestions && getEmailSuggestions().length > 0
                          ? `login-suggestion-${suggestionHighlight}`
                          : undefined
                      }
                      className={cn(
                        ...
                        formError
                          ...
                          : "hover:border-[#D6D3D1] dark:hover:border-slate-600",
                      )}
                      aria-invalid={formError ? "true" : "false"}
                      aria-describedby={
                        formError ? "login-email-error" : undefined
                      }
                    />
                    {showSuggestions && getEmailSuggestions().length > 0 && (
                      <div
                          ...
                          .map((suggestion, index) => (
                            <button
                              ...
                              className={cn(
                                ...
                                index === suggestionHighlight
                                  ? "bg-[#F7F6F3] dark:bg-slate-700"
                                  : "hover:bg-[#F7F6F3] dark:hover:bg-slate-700",
                              )}
                            >
                              {suggestion}
                            </button>
                ...
                <button
                  ...
                  aria-label={
                    isLoading ? "Sending magic link..." : "Continue to sign in"
                  }
                  aria-busy={isLoading}
              ...
              <div className="flex items-center justify-center gap-4 py-3">
                <div
                  className="flex items-center gap-1.5 text-xs text-[#787774] bg-white px-3 py-1.5 rounded-full border border-[#E9E9E7]"
              ...
              <div className="text-center pt-4 border-t border-[#E9E9E7]">
                <p className="text-xs text-[#787774]">
                  ...
                  <Link
                    to="/terms"
                    className="text-[#455DD3] hover:underline font-medium"
                  >
                    Terms
                  </Link>{" "}
                  and{" "}
