...
export default function PricingVs() {
  ...
  const COMPETITORS_MAP: Record<string, CompetitorData> = Object.fromEntries(
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 selection:bg-primary-500/20 selection:text-primary-700">
      <SEO
        ...
        breadcrumbs={[
          ...
          {
            name: competitor.name + " vs JobHuntin Pricing",
            url: canonicalUrl,
          },
        ]}
        schema={[
      ...
      <main className="max-w-5xl mx-auto px-6 py-24">
        ...
        >
          ...
          <h1 className="text-4xl md:text-6xl font-sans font-black mb-6 leading-tight text-slate-900">
            ...
            <span className="text-primary-600 font-black">JobHuntin</span>
          </h1>
          <p className="text-xl text-slate-500 max-w-2xl mx-auto font-medium">
            {competitor.name} starts at {competitor.pricing?.starts_at}.
            JobHuntin is free to start with Pro at $19/mo. See what you get for
            your money.
          </p>
        </motion.div>
        ...
        >
              ...
              <ul className="space-y-2">
                {!competitor.features?.stealth_mode && (
                  <li className="flex items-center gap-2 text-sm text-red-500">
                    <X className="w-4 h-4" /> Stealth Mode
                  </li>
                )}
                {!competitor.features?.resume_tailoring && (
                  <li className="flex items-center gap-2 text-sm text-red-500">
                    <X className="w-4 h-4" /> Per-app resume tailoring
                  </li>
                )}
                {!competitor.features?.ai_agent && (
                  <li className="flex items-center gap-2 text-sm text-red-500">
                    <X className="w-4 h-4" /> Autonomous AI agent
                  </li>
                )}
                {!competitor.features?.auto_apply && (
                  <li className="flex items-center gap-2 text-sm text-red-500">
                    <X className="w-4 h-4" /> Auto-apply
                  </li>
                )}
              </ul>
            </div>
          ...
          <div className="bg-primary-50 rounded-3xl border-2 border-primary-200 p-8 shadow-sm relative">
            ...
            <div className="space-y-4">
                {JOBHUNTIN_TIERS.map((tier, index) => (
                  <div key={index} className="bg-white rounded-2xl p-5 border border-primary-100">
            ...
            <table className="w-full min-w-[480px]">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50/50">
                  ...
                  <th className="px-6 py-4 text-center text-sm font-bold text-primary-600">
                    JobHuntin Pro
                  </th>
                  <th className="px-6 py-4 text-center text-sm font-bold text-slate-400">
                    {competitor.name}
                  </th>
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  [
                    ...
                    true,
                    competitor.features?.cover_letter_gen,
                  ],
                  [
                    "ATS Optimization",
                    true,
                    competitor.features?.ats_optimization,
                  ],
                  [
                    "Job Tracking Dashboard",
                    true,
                    competitor.features?.job_tracking,
                  ],
                  [
                    "Chrome Extension",
                    true,
                    competitor.features?.browser_extension,
                  ],
                  ["Mobile App", true, competitor.features?.mobile_app],
                  ].map(([feature, jh, comp], index) => (
                ...
                <tr className="bg-primary-50 border-t-2 border-primary-200">
                  ...
                  <td className="px-6 py-4 text-center text-lg font-black text-primary-600">
                    $19/mo
                  </td>
                  <td className="px-6 py-4 text-center text-lg font-bold text-slate-500">
                    {competitor.pricing?.starts_at}
                  </td>
                </tr>
              </tbody>
