...
async def install_blueprint(
    ...
    async with db.acquire() as conn:
        ...
        if not bp:
            raise HTTPException(status_code=404, detail="Blueprint not found or not approved")
        ...
        row = await conn.fetchrow(
            ...
            RETURNING *
            """,
            blueprint_id, ctx.tenant_id, ctx.user_id, bp["version"], json.dumps(body.config),
        )
        ...
        await record_audit_event(
            ...
            resource="blueprint", resource_id=blueprint_id,
...
async def submit_blueprint(
    ...
    if ctx.plan not in ("PRO", "TEAM", "ENTERPRISE"):
        raise HTTPException(status_code=403, detail="Blueprint submission requires a paid plan")

    async with db.acquire() as conn:
        existing = await conn.fetchval(
            "SELECT id FROM public.marketplace_blueprints WHERE slug = $1", body.slug,
        )
        if existing:
            raise HTTPException(status_code=409, detail=f"Slug '{body.slug}' already taken")

        tenant = await conn.fetchrow("SELECT name FROM public.tenants WHERE id = $1", ctx.tenant_id)
        ...
        row = await conn.fetchrow(
            ...
            """,
            body.slug, body.name, body.description, body.long_description,
            body.category, ctx.tenant_id, author_name, body.version,
            json.dumps(body.source_code), json.dumps(body.config_schema), body.price_cents,
        )

        await record_audit_event(
            conn, ctx.tenant_id, ctx.user_id,
            action="marketplace.blueprint_submitted",
            resource="blueprint", resource_id=str(row["id"]),
...
async def review_blueprint(
    ...
    async with db.acquire() as conn:
        await conn.execute(
            ...
            """,
            blueprint_id, ctx.tenant_id, ctx.user_id, body.rating, body.review_text,
        )
