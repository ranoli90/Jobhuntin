...
async def submit_application(
    ...
    try:
        ...
        if not job_id:
            job_id = await conn.fetchval(
                ...
                body.job_url,
                body.metadata.get("title", "API Submission"),
                tenant_id,
            )
    ...
    await record_api_usage(
        db, str(api_key["id"]), tenant_id, "/applications", "POST", 201, latency
    )
...
async def batch_submit(
    ...
    for app_req in body.applications:
        try:
            ...
            results.append(
                {"job_url": app_req.job_url, "success": False, "error": str(exc)}
            )
...
async def staffing_bulk_submit(
    ...
    if api_key.get("tenant_plan") not in ("TEAM", "ENTERPRISE"):
        raise HTTPException(
            status_code=403, detail="Staffing API requires TEAM or ENTERPRISE plan"
        )
    ...
    priority = compute_priority_score(
        api_key.get("tenant_plan", "ENTERPRISE"), is_bulk=True
    )
    ...
    async with db.acquire() as conn:
        batch = await conn.fetchrow(
            ...
            len(body.candidates),
            s.staffing_price_per_submit_cents,
            s.staffing_base_monthly_cents,
        )
    ...
    latency = int((time.time() - start) * 1000)
    await record_api_usage(
        db, str(api_key["id"]), tenant_id, "/staffing/bulk-submit", "POST", 201, latency
    )
    incr(
        "api_v2.staffing.batch_created", tags={"candidates": str(len(body.candidates))}
    )
    ...
    return {
        ...
        "estimated_cost_cents": len(body.candidates)
        * s.staffing_price_per_submit_cents,
    }
